...
class ViewerWidget;
class QtOSGViewer;
typedef tools::shared_ptr<QtOSGViewer> QtOSGViewerPtr;
typedef tools::weak_ptr<QtOSGViewer> QtOSGViewerWeakPtr;
typedef tools::shared_ptr<QtOSGViewer const> QtOSGViewerConstPtr;
...
public:
    ...
    tools::shared_ptr<EnvironmentMutex::scoped_try_lock> LockEnvironment(uint64_t timeout=50000,bool bUpdateEnvironment = true);
...
public:
        ...
        inline void Call() {
            ...
            tools::shared_ptr<void> finishfn((void*)0, tools::bind(&GUIThreadFunction::SetFinished, this));
...
private:
    ...
    typedef tools::shared_ptr<GUIThreadFunction> GUIThreadFunctionPtr;
...
public:
        ...
        virtual ~PrivateGraphHandle() {
            tools::shared_ptr<QtOSGViewer> viewer = _wviewer.lock();
            if(!!viewer) {
                viewer->_PostToGUIThread(tools::bind(&QtOSGViewer::_CloseGraphHandle, viewer, _handle)); // _handle is copied, so it will maintain the reference
            }
        }
        ...
        virtual void SetTransform(const RaveTransform<float>& t)
        {
            tools::shared_ptr<QtOSGViewer> viewer = _wviewer.lock();
            if(!!viewer) {
                viewer->_PostToGUIThread(tools::bind(&QtOSGViewer::_SetGraphTransform, viewer, _handle, t)); // _handle is copied, so it will maintain the reference
            }
        }
        ...
        virtual void SetShow(bool bShow)
        {
            tools::shared_ptr<QtOSGViewer> viewer = _wviewer.lock();
            if(!!viewer) {
                viewer->_PostToGUIThread(tools::bind(&QtOSGViewer::_SetGraphShow, viewer, _handle, bShow)); // _handle is copied, so it will maintain the reference
            }
        }
    ...
    inline QtOSGViewerPtr shared_viewer() {
        return tools::static_pointer_cast<QtOSGViewer>(shared_from_this());
    }
    ...
    inline QtOSGViewerConstPtr shared_viewer_const() const {
        return tools::static_pointer_cast<QtOSGViewer const>(shared_from_this());
    }
