...
namespace {
	...
	const std::string outlineFragStr =
			...
			"}\n"
			""
			"float gradientIntensity(inout vec4 n[4]) {\n"
			"    float h = 1;\n"
			"\n"
			"    vec3 xm = n[0].rgb;\n"
			"    vec3 xp = n[1].rgb;\n"
			...
			"{\n"
			"    return 1.0/(1+exp(-4*(value-1.6)));\n"
			"}\n"
			"float delta(float x0)\n"
			"{\n"
			"    return exp(-pow(((x0-1)/0.8),2));\n"
			"}\n"
			...
			"    gl_FragColor = vec4(0,0,0.0, intensity);\n"
			"}\n";

	const std::string diladionVertStr =
			"#version 120\n"
			"void main()\n"
			"{\n"
			"	// Vertex position in main camera Screen space.\n"
	...
	const std::string dilationFragStr =
			"#version 120\n"
			"#extension GL_ARB_texture_multisample : enable \n"
			...
			"	 float maxVal = 0;\n"
			"	 for(int i=0; i < 9; ++i){\n"
			"	 	maxVal = max(samples[i].a, maxVal);\n"
			"	 }\n"
			"    gl_FragColor = samples[4] + vec4(0,0,0.0, maxVal);\n"
			"}\n";
...
{
	// if(colorFboTexture) {
	// 	colorFboTexture->setTextureSize(width, height);
	// 	colorFboTexture->dirtyTextureObject();
	// }
	if(camera) {
...
inline RenderPassState* createFirstRenderPass(osg::ref_ptr<osg::Camera> mainSceneCamera, osg::ref_ptr<osg::Node> mainSceneRoot)
{
	...
	renderPassState->colorFboTexture = RenderUtils::CreateFloatTextureRectangle(1024, 768);
	...
	renderPassState->camera->setClearColor(osg::Vec4(0, 0, 0, 1.0));
...
inline RenderPassState* createThirdRenderPass(RenderPassState* secondPassState)
{
	...
	RenderUtils::SetShaderProgramOnStateSet(thirdPassStateSet.get(), diladionVertStr, dilationFragStr);
...
osg::ref_ptr<osg::Group> OutlineShaderPipeline::CreateOutlineSceneFromOriginalScene(osg::ref_ptr<osg::Camera> mainSceneCamera, osg::ref_ptr<osg::Node> mainSceneRoot)
{
	...
	osg::ref_ptr<osg::Group> pipelineGroup = new osg::Group();
	pipelineGroup->addChild(_renderPassStates[0]->camera.get());
	pipelineGroup->addChild(_renderPassStates[1]->camera.get());
	pipelineGroup->addChild(mainSceneRoot);
	pipelineGroup->addChild(_renderPassStates[2]->camera.get());

	return pipelineGroup.release();
