...
#ifndef OPENRAVE_OSGFX_CARTOON_
#define OPENRAVE_OSGFX_CARTOON_
 ...
 */
class OpenRAVECartoon : public osgFX::Effect {
public:
    OpenRAVECartoon();
    OpenRAVECartoon(const OpenRAVECartoon& copy, const osg::CopyOp& copyop = osg::CopyOp::SHALLOW_COPY);
...
protected:
    virtual ~OpenRAVECartoon() {
    }
    OpenRAVECartoon& operator=(const OpenRAVECartoon&) {
        return *this;
    }
...
inline void OpenRAVECartoon::setOutlineLineWidth(float w)
{
    _wf_lw->setWidth(w);
}

inline int OpenRAVECartoon::getLightNumber() const
{
    return _lightnum;
}

inline void OpenRAVECartoon::setLightNumber(int n)
{
    _lightnum = n;
    dirtyTechniques();
}
