...
void IvObjectDragger::CheckCollision(bool flag)
{
    ...
    if (_checkCollision && !!selectedItem) {
        ...
        KinBodyItemPtr pbody = tools::dynamic_pointer_cast<KinBodyItem>(selectedItem);
        if( !!pbody ) {
...
{
    KinBodyItemPtr pbody = tools::dynamic_pointer_cast<KinBodyItem>(pItem);
    BOOST_ASSERT( !!pItem );
...
void IvJointDragger::UpdateSkeleton()
{
    ...
    RobotItemPtr probotitem = tools::dynamic_pointer_cast<RobotItem>(pbody);
