...
void IvObjectDragger::CheckCollision(bool flag)
{
    ...
    if (_checkCollision && !!selectedItem) {
        ...
        KinBodyItemPtr pbody = OPENRAVE_DYNAMIC_POINTER_CAST<KinBodyItem>(selectedItem);
        if( !!pbody ) {
...
{
    KinBodyItemPtr pbody = OPENRAVE_DYNAMIC_POINTER_CAST<KinBodyItem>(pItem);
    BOOST_ASSERT( !!pItem );
...
void IvJointDragger::UpdateSkeleton()
{
    ...
    RobotItemPtr probotitem = OPENRAVE_DYNAMIC_POINTER_CAST<RobotItem>(pbody);
