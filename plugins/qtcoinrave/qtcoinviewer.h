...
public:
    ...
    virtual void Reset();
    virtual tools::shared_ptr<void> LockGUI();
...
public:
    class EnvMessage : public tools::enable_shared_from_this<EnvMessage>
    {
public:
...
protected:
        tools::weak_ptr<QtCoinViewer> _pviewer;
        void** _ppreturn;
        boost::mutex _mutex;
        tools::shared_ptr<boost::mutex::scoped_lock> _plock;
    };
    typedef tools::shared_ptr<EnvMessage> EnvMessagePtr;
    typedef tools::shared_ptr<EnvMessage const> EnvMessageConstPtr;
...
protected:
    ...
    {
public:
        PrivateGraphHandle(tools::weak_ptr<QtCoinViewer> wviewer, SoSwitch* handle) : _handle(handle), _wviewer(wviewer) {
            BOOST_ASSERT(_handle!=NULL);
        }
        virtual ~PrivateGraphHandle() {
            tools::shared_ptr<QtCoinViewer> viewer = _wviewer.lock();
            if( !!viewer ) {
        ...
        SoSwitch* _handle;
        tools::weak_ptr<QtCoinViewer> _wviewer;
    };

    inline QtCoinViewerPtr shared_viewer() {
        return tools::dynamic_pointer_cast<QtCoinViewer>(shared_from_this());
    }
    ...
    inline QtCoinViewerConstPtr shared_viewer_const() const {
        return tools::dynamic_pointer_cast<QtCoinViewer const>(shared_from_this());
    }
    ...
    tools::shared_ptr<IvDragger>    _pdragger;
    std::list< tools::shared_ptr<IvDragger> > _plistdraggers;     /// draggers drawn
...
/*class ScreenRendererWidget : public QWidget
   ...
   private:
    ...
    tools::shared_ptr<QtCoinViewer> _openraveviewer;
