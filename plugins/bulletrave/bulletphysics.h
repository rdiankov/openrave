...
public:
    ...
    inline std::shared_ptr<BulletPhysicsEngine> shared_physics() {
        return std::dynamic_pointer_cast<BulletPhysicsEngine>(shared_from_this());
    }
    inline std::shared_ptr<BulletPhysicsEngine const> shared_physics_const() const {
        return std::dynamic_pointer_cast<BulletPhysicsEngine const>(shared_from_this());
    }

class PhysicsPropertiesXMLReader : public BaseXMLReader
    {
public:
        PhysicsPropertiesXMLReader(std::shared_ptr<BulletPhysicsEngine> physics, const AttributesList& atts) : _physics(physics) {
        }
...
protected:
        ...
        std::shared_ptr<BulletPhysicsEngine> _physics;
        stringstream _ss;
    };
...
public:
    ...
    {
    	return BaseXMLReaderPtr(new PhysicsPropertiesXMLReader(std::dynamic_pointer_cast<BulletPhysicsEngine>(ptr),atts));
    }
	...
	__description = ":Interface Authors: Max Argus, Nick Hillier, Katrina Monkley, Rosen Diankov\n\nInterface to `Bullet Physics Engine <http://bulletphysics.org/>`_\n";
        RegisterCommand("SetStaticBodyTransform",std::bind(&BulletPhysicsEngine::SetStaticBodyTransform,this,_1,_2),"Sets the transformation of a static body manually, not allowed to use for dynamic bodies and it should be used with caution even for static bodies because it can cause instabilities in physics engine.");
	...
	std::shared_ptr<btRigidBody> rigidbody = std::dynamic_pointer_cast<btRigidBody>(_space->GetLinkBody(plink));
    ...
    virtual bool InitEnvironment()
    {
        ...
        _space->SetSynchronizationCallback(std::bind(&BulletPhysicsEngine::_SyncCallback, shared_physics(),_1));
    ...
    virtual bool SetLinkVelocity(KinBody::LinkPtr plink, const Vector& linearvel, const Vector& angularvel)
    {
        ...
        std::shared_ptr<btRigidBody> rigidbody(pinfo->vlinks.at(plink->GetIndex())->_rigidbody);
        if( !rigidbody ) {
    ...
    virtual bool GetLinkVelocities(KinBodyConstPtr pbody, std::vector<std::pair<Vector,Vector> >& velocities)
    {
        ...
        FOREACHC(itlink, pbody->GetLinks()) {
            std::shared_ptr<btRigidBody> rigidbody = std::dynamic_pointer_cast<btRigidBody>(_space->GetLinkBody(*itlink));
            if(!!rigidbody) {
                btVector3 pf = rigidbody->getLinearVelocity();
    ...
    {
        std::shared_ptr<btTypedConstraint> joint = _space->GetJoint(pjoint);
      ...
      virtual bool AddJointTorque(KinBody::JointPtr pjoint, const std::vector<dReal>& pTorques)
    {
       ...
       switch(joint->getConstraintType()) {
        ...
        case D6_CONSTRAINT_TYPE:{
            ...
            std::shared_ptr<btGeneric6DofConstraint> d6joint = std::dynamic_pointer_cast<btGeneric6DofConstraint>(joint);
        ...
        case HINGE_CONSTRAINT_TYPE: {
            std::shared_ptr<btHingeConstraint> hingejoint = std::dynamic_pointer_cast<btHingeConstraint>(joint);
            btTransform transB = bodyB.getWorldTransform();
        ...
        case SLIDER_CONSTRAINT_TYPE: {
            std::shared_ptr<btSliderConstraint> slidejoint = std::dynamic_pointer_cast<btSliderConstraint>(joint);
...
private:
    ...
    {
        return std::dynamic_pointer_cast<BulletSpace::KinBodyInfo>(pbody->GetUserData("bulletphysics"));
    }
    ...
    int _options;
    std::shared_ptr<BulletSpace> _space;
    std::shared_ptr<btDiscreteDynamicsWorld> _dynamicsWorld;
    ...
    std::shared_ptr<btOverlapFilterCallback> _filterCallback;
