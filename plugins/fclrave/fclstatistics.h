...
static std::vector<OpenRAVE::tools::weak_ptr<FCLStatistics> > globalStatistics;
...
public:
    ...
    void DisplayAll() {
        FOREACH(itstat, globalStatistics) {
            OpenRAVE::tools::shared_ptr<FCLStatistics> pstat = itstat->lock();
            if( pstat ) {
                pstat->Display();
            }
        }
...
typedef OpenRAVE::tools::shared_ptr<FCLStatistics> FCLStatisticsPtr;

#define SETUP_STATISTICS(statistics, userdatakey, id) \
    statistics = OpenRAVE::tools::make_shared<FCLStatistics>(userdatakey, id); \
    globalStatistics.push_back(OpenRAVE::tools::weak_ptr<FCLStatistics>(statistics))
