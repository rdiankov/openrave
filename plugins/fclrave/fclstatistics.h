...
static std::vector<tools::weak_ptr<FCLStatistics> > globalStatistics;
...
public:
    ...
    void DisplayAll() {
        FOREACH(itstat, globalStatistics) {
            tools::shared_ptr<FCLStatistics> pstat = itstat->lock();
            if( pstat ) {
                pstat->Display();
            }
        }
...
typedef tools::shared_ptr<FCLStatistics> FCLStatisticsPtr;

#define SETUP_STATISTICS(statistics, userdatakey, id) \
    statistics = tools::make_shared<FCLStatistics>(userdatakey, id); \
    globalStatistics.push_back(tools::weak_ptr<FCLStatistics>(statistics))
