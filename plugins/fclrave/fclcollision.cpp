...
void FCLCollisionChecker::_AppendNarrowPhaseLinkInfos(KinBodyConstPtr pbody, bool bactiveDOFs, std::vector<const FCLSpace::FCLKinBodyInfo::LinkInfo*>& vLinkInfos)
{
    ...
    pbody->GetEnv()->GetBodiesFromEnvironmentBodyIndices(vecAttachedEnvBodyIndices, vecAttachedBodies);

    std::vector<uint8_t> vActiveLinks; // 1 if the link is affected by an active DOF
    FOREACH(itbody, vecAttachedBodies) {
        const KinBodyPtr& pAttachedBody = *itbody;
        if( !pAttachedBody ) {
            continue;
        }
        ...
        if( !pinfo ) {
            continue; // not initialized in this checker, skip
        }

        // when tracking active DOFs, only the tracking robot's active links are considered (same as the broad-phase body manager)
        const bool bComputeActiveLinks = bactiveDOFs && pAttachedBody == pbody && attachedBody.IsRobot();
        if( bComputeActiveLinks ) {
            RobotBaseConstPtr probot = OpenRAVE::RaveInterfaceConstCast<RobotBase>(pAttachedBody);
            const RobotBase& robot = *probot;
            vActiveLinks.assign(robot.GetLinks().size(), 0);
        ...
        FOREACH(itlink, attachedBody.GetLinks()) {
            ...
            if( bComputeActiveLinks && !vActiveLinks.at(linkIndex) ) {
                continue;
            }
            if( linkIndex >= (int)pinfo->vlinks.size() ) {
                continue;
            }
            const boost::shared_ptr<FCLSpace::FCLKinBodyInfo::LinkInfo>& plinkInfo = pinfo->vlinks[linkIndex];
            if( !!plinkInfo && !!plinkInfo->linkBV.second ) {
                vLinkInfos.push_back(plinkInfo.get());
            }
        }
    }
}

bool FCLCollisionChecker::CheckNarrowPhaseBodyCollision(KinBodyConstPtr pbody1, KinBodyConstPtr pbody2, CollisionReportPtr report)
{
    START_TIMING_OPT(_statistics, "Body/Body/NarrowPhase",_options,(pbody1->IsRobot() || pbody2->IsRobot()));
    if( !!report ) {
        report->Reset(_options);
    }
    ...
    // Collect the links (with their bounding volumes / geometries) of each body and its attached
    // bodies, then directly run the narrow-phase fcl collision between each candidate geometry pair,
    // instead of going through the broad-phase collision managers.
    std::vector<const FCLSpace::FCLKinBodyInfo::LinkInfo*> vLinkInfos1, vLinkInfos2;
    _AppendNarrowPhaseLinkInfos(pbody1, !!(_options & OpenRAVE::CO_ActiveDOFs), vLinkInfos1);
    _AppendNarrowPhaseLinkInfos(pbody2, false, vLinkInfos2); // matching CheckCollision(body,body): active DOFs are not applied to pbody2

    if( vLinkInfos1.empty() || vLinkInfos2.empty() ) {
        return false;
    }
    ...
    const std::vector<LinkConstPtr> vlinkexcluded;
    CollisionCallbackData query(shared_checker(), report, vbodyexcluded, vlinkexcluded);
    if( (_options & OpenRAVE::CO_Distance) && !report ) {
        throw openrave_exception("FCLCollision - ERROR: YOU MUST PASS IN A CollisionReport STRUCT TO MEASURE DISTANCE!\n");
    }
    ADD_TIMING(_statistics);

    // pbody1 and pbody2 are not attached (checked above), so their link sets are disjoint and no
    // per-pair attachment check is needed.
    FOREACH(itLINK1, vLinkInfos1) {
        const FCLSpace::FCLKinBodyInfo::LinkInfo& LINK1 = **itLINK1;
        FOREACH(itLINK2, vLinkInfos2) {
            const FCLSpace::FCLKinBodyInfo::LinkInfo& LINK2 = **itLINK2;
            if( !(_options & OpenRAVE::CO_Distance) && !LINK1.linkBV.second->getAABB().overlap(LINK2.linkBV.second->getAABB()) ) {
                continue;
            }
            FOREACH(itgeom1, LINK1.vgeoms) {
                FOREACH(itgeom2, LINK2.vgeoms) {
                    // if( _options & OpenRAVE::CO_Distance ) {
