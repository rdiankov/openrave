...
class MobyPhysics : public PhysicsEngineBase
{
    ...
    inline std::shared_ptr<MobyPhysics> shared_physics() {
        return std::dynamic_pointer_cast<MobyPhysics>(shared_from_this());
    }

    inline std::shared_ptr<MobyPhysics const> shared_physics_const() const {
        return std::dynamic_pointer_cast<MobyPhysics const>(shared_from_this());
    }

    class PhysicsPropertiesXMLReader : public BaseXMLReader
    {
    public:
        PhysicsPropertiesXMLReader(std::shared_ptr<MobyPhysics> physics, const AttributesList& atts) : _physics(physics) {
        }
...
protected:
        ...
        std::shared_ptr<MobyPhysics> _physics;
        stringstream _ss;
    };
...
public:
    ...
    {
    	return BaseXMLReaderPtr(new PhysicsPropertiesXMLReader(std::dynamic_pointer_cast<MobyPhysics>(ptr),atts));
    }
    ...
    virtual bool InitEnvironment()
    {
        ...
        _space->SetSynchronizationCallback(std::bind(&MobyPhysics::_SyncCallback, shared_physics(),_1));

        // +basic simulator
        //_sim.reset(new Moby::Simulator());
        //_sim->integrator = std::shared_ptr<Moby::Integrator>(new Moby::EulerIntegrator());
        // -basic simulator
    ...
    virtual bool SetBodyForce(KinBody::LinkPtr plink, const Vector& force, const Vector& position, bool bAdd)
    {
        ...
        std::shared_ptr<Ravelin::Pose3d> pose(new Ravelin::Pose3d(Ravelin::Quatd(0,0,0,1), _space->GetRavelinOrigin(position), Moby::GLOBAL));
    ...
    dReal _StepSize;
    Vector _gravity;
    std::shared_ptr<MobySpace> _space;

private:
    static MobySpace::KinBodyInfoPtr GetPhysicsInfo(KinBodyConstPtr pbody)
    {
        return std::dynamic_pointer_cast<MobySpace::KinBodyInfo>(pbody->GetUserData("mobyphysics"));
    }
    ...
    int _options;
    std::shared_ptr<Moby::Simulator> _sim; 
};
