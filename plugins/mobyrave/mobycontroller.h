...
class MobyController : public ControllerBase
{
    ...
    {
    public:
        ControllerPropertiesXMLReader(tools::shared_ptr<MobyController> controller, const AttributesList& atts) : _controller(controller) {
        }
...
protected:
        ...
        tools::shared_ptr<MobyController> _controller;
        stringstream _ss;
    };
...
public:
    ...
    {
    	return BaseXMLReaderPtr(new ControllerPropertiesXMLReader(tools::dynamic_pointer_cast<MobyController>(ptr),atts));
    }
    ...
    MobyController(EnvironmentBasePtr penv, std::istream& sinput) : ControllerBase(penv), cmdid(0), _bPause(false), _bIsDone(true), _bCheckCollision(false), _bThrowExceptions(false), _bEnableLogging(false), _bEnableTuningLogging(false), _bSteadyState(false)    {
        ...
        RegisterCommand("Pause",tools::bind(&MobyController::_Pause,this,_1,_2),
                        "pauses the controller from reacting to commands ");
        RegisterCommand("SetCheckCollisions",tools::bind(&MobyController::_SetCheckCollisions,this,_1,_2),
                        "If set, will check if the robot gets into a collision during movement");
        RegisterCommand("SetThrowExceptions",tools::bind(&MobyController::_SetThrowExceptions,this,_1,_2),
                        "If set, will throw exceptions instead of print warnings. Format is:\n\n  [0/1]");
        RegisterCommand("SetEnableLogging",tools::bind(&MobyController::_SetEnableLogging,this,_1,_2),
                        "If set, will write trajectories to disk");
        RegisterCommand("SetEnableTuningLogging",tools::bind(&MobyController::_SetEnableTuningLogging,this,_1,_2),
    ...
    virtual bool Init(RobotBasePtr robot, const std::vector<int>& dofindices, int nControlTransformation)
    {
        ...
        if( !!_probot ) {
            ...
            _cblimits = _probot->RegisterChangeCallback(KinBody::Prop_JointLimits|KinBody::Prop_JointAccelerationVelocityTorqueLimits,tools::bind(&MobyController::_SetJointLimits,tools::bind(&utils::sptr_from<MobyController>, weak_controller())));
            _SetJointLimits();
...
private:
    ...
    inline tools::shared_ptr<MobyController> shared_controller()
    {
        return tools::dynamic_pointer_cast<MobyController>(shared_from_this());
    }

    inline tools::shared_ptr<MobyController const> shared_controller_const() const
    {
        return tools::dynamic_pointer_cast<MobyController const>(shared_from_this());
    }

    inline tools::weak_ptr<MobyController> weak_controller()
    {
