...
typedef void (*ControllerCallbackFn)(std::shared_ptr<Moby::DynamicBody>,double,void*);

// manages a space of Moby objects
class MobySpace : public std::enable_shared_from_this<MobySpace>
{
    inline std::weak_ptr<MobySpace> weak_space() {
...
public:
        ...
        KinBodyInfo(std::shared_ptr<Moby::Simulator> world, bool bPhysics) : _world(world), _bPhysics(bPhysics) 
        {
            nLastStamp = 0;
        }
        ...
        int nLastStamp;
        vector<std::shared_ptr<LINK> > vlinks;     
        UserDataPtr _geometrycallback;
        std::weak_ptr<MobySpace> _mobyspace;
private:
        std::shared_ptr<Moby::Simulator> _world;
        bool _bPhysics;
    };
    ...
    bool InitEnvironment(std::shared_ptr<Moby::Simulator> world)
    {
        _world = world;
    ...
    inline std::shared_ptr<KinBodyInfo::LINK> DeriveLink(boost::shared_ptr<KinBody::Link> plink) {

        std::shared_ptr<KinBodyInfo::LINK> link;
        link = std::shared_ptr<KinBodyInfo::LINK>(new KinBodyInfo::LINK());
    ...
    inline Moby::JointPtr DeriveJoint(KinBodyInfoPtr pinfo, std::shared_ptr<KinBody::Joint> pjoint) {
        ...
        switch(pjoint->GetType()) 
        {
            case KinBody::JointHinge: 
            {
                ...
                std::shared_ptr<Moby::RevoluteJoint> rjoint(new Moby::RevoluteJoint);
...
/*
                ...
                std::shared_ptr<Moby::PrismaticJoint> sjoint(new Moby::PrismaticJoint);
...
*/
            ...
            {
                std::shared_ptr<Moby::SphericalJoint> sjoint(new Moby::SphericalJoint);
                joint = sjoint;
                break;
            }
            case KinBody::JointUniversal: 
            {
                std::shared_ptr<Moby::UniversalJoint> sjoint(new Moby::UniversalJoint);
                joint = sjoint;
	...
	pbody->SetDOFValues(vzeros);
        ...
        if(pbody->GetLinks().size() == 1) 
        {
            ...
            std::shared_ptr<KinBody::Link> plink = pbody->GetLinks()[0];

            // initialize the Moby compatible link by deriving from the OpenRave link
            std::shared_ptr<KinBodyInfo::LINK> link = DeriveLink(plink);
            ...
            link->register_controller_callback( std::bind( &MobySpace::_Controller,shared_from_this(),_1,_2,_3) );
        ...
        else if(pbody->GetLinks().size() > 1)
        {
            ...
            vector<std::shared_ptr<KinBodyInfo::LINK> > veclinks;
            ...
            {
                // initialize the Moby compatible link by deriving from the OpenRave link
                std::shared_ptr<KinBodyInfo::LINK> link = DeriveLink(*itlink);
            ...
            // register the controller callback; necessary for writing state and controls
            morcab->register_controller_callback( std::bind( &MobySpace::_Controller,shared_from_this(),_1,_2,_3) );
...
*/
    ...
    static inline Ravelin::SForced GetRavelinSForce(const Vector& force, const Vector& torque, std::shared_ptr<const Ravelin::Pose3d> pose)
    {
...
private:
    ...
    virtual void GeometryChangedCallback(KinBodyWeakPtr _pbody)
    {
        ...
        BOOST_ASSERT(std::shared_ptr<MobySpace>(pinfo->_mobyspace) == shared_from_this());
        BOOST_ASSERT(pinfo->pbody==pbody);
    ...
    void _Controller( Moby::DynamicBodyPtr db, const double& t, void* ) {
        ...
        Moby::ArticulatedBodyPtr ab = std::dynamic_pointer_cast<Moby::ArticulatedBody>(db);
        if(!!ab) 
        {
        ...
        Moby::RigidBodyPtr rb = std::dynamic_pointer_cast<Moby::RigidBody>(db);
        if(!!rb) 
        {
...
public:
    std::shared_ptr<Moby::GravityForce> _gravity;
