...
class IkFastModule : public ModuleBase
{
    ...
    class IkLibrary : public tools::enable_shared_from_this<IkLibrary>
    {
public:
        template <typename T>
...
public:
            ...
            tools::shared_ptr<IkLibrary> _library;
        };
...
#ifdef OPENRAVE_IKFAST_FLOAT32
            if( realsize == 4 ) {
                tools::shared_ptr<MyFunctions<float> > ikfunctions(new MyFunctions<float>());
...
#endif
            if( realsize == 8 ) {
                tools::shared_ptr<MyFunctions<double> > ikfunctions(new MyFunctions<double>());
                _InitFunctions(ikfunctions);
        ...
        template <typename T>
        bool _InitFunctions(tools::shared_ptr<MyFunctions<T> > ikfunctions)
        {
...
#ifdef OPENRAVE_IKFAST_FLOAT32
            if( !!_ikfloat ) {
                tools::shared_ptr<MyFunctions<float> > newfunctions(new MyFunctions<float>(*_ikfloat));
                newfunctions->_library = shared_from_this();
                return CreateIkFastSolver(penv,ss,newfunctions,vfreeinc);
            }
#endif
            if( !!_ikdouble ) {
                tools::shared_ptr<MyFunctions<double> > newfunctions(new MyFunctions<double>(*_ikdouble));
                newfunctions->_library = shared_from_this();
                return CreateIkFastSolver(penv,ss,newfunctions,vfreeinc);
            }
...
#ifdef OPENRAVE_IKFAST_FLOAT32
        tools::shared_ptr<MyFunctions<float> > _ikfloat;
#endif
        tools::shared_ptr<MyFunctions<double> > _ikdouble;
private:
    ...
    inline tools::shared_ptr<IkFastModule> shared_problem() {
        return tools::dynamic_pointer_cast<IkFastModule>(shared_from_this());
    }
    inline tools::shared_ptr<IkFastModule const> shared_problem_const() const {
        return tools::dynamic_pointer_cast<IkFastModule const>(shared_from_this());
    }
...
public:
    IkFastModule(EnvironmentBasePtr penv, std::istream& sinput) : ModuleBase(penv)
    {
        ...
        RegisterCommand("AddIkLibrary",tools::bind(&IkFastModule::AddIkLibrary,this,_1,_2),
...
#ifdef Boost_IOSTREAMS_FOUND
        RegisterCommand("LoadIKFastSolver",tools::bind(&IkFastModule::LoadIKFastSolver,this,_1,_2),
...
#endif
        RegisterCommand("PerfTiming",tools::bind(&IkFastModule::PerfTiming,this,_1,_2),
        ...
        RegisterCommand("IKTest",tools::bind(&IkFastModule::IKtest,this,_1,_2),
                        "Tests for an IK solution if active manipulation has an IK solver attached");
        RegisterCommand("DebugIK",tools::bind(&IkFastModule::DebugIK,this,_1,_2),
...
* string robot - name of the robot to test. the active manipulator of the roobt is used.\n\n");
    ...
    bool AddIkLibrary(ostream& sout, istream& sinput)
    {
        ...
        tools::shared_ptr<IkLibrary> lib = _AddIkLibrary(ikname,libraryname);
        if( !lib ) {
            return false;
        }
        sout << lib->GetIKType();
    ...
    tools::shared_ptr<IkLibrary> _AddIkLibrary(const string& ikname, const string& _libraryname)
    {
#ifdef HAVE_BOOST_FILESYSTEM
        ...
        tools::shared_ptr<IkLibrary> lib;
        ...
        if( !lib ) {
            ...
            if( !lib->Init(ikname, libraryname) ) {
                return tools::shared_ptr<IkLibrary>();
            }
    ...
    bool LoadIKFastSolver(ostream& sout, istream& sinput)
    {
        ...
        for(int iter = 0; iter < 2; ++iter) {
            ...
            tools::shared_ptr<IkLibrary> lib = _AddIkLibrary(ikfastname,ikfilenamefound);
            bool bsuccess = true;
            if( !lib ) {
                bsuccess = false;
            }
    ...
    bool PerfTiming(ostream& sout, istream& sinput)
    {
        ...
        tools::shared_ptr<IkLibrary> lib(new IkLibrary());
    ...
    template<typename T> bool _PerfTiming(ostream& sout, tools::shared_ptr<ikfast::IkFastFunctions<T> > ikfunctions, int num, dReal maxtime)
    {
    ...
    static list< tools::shared_ptr<IkLibrary> >*& GetLibraries()
    {
        static list< tools::shared_ptr<IkLibrary> >* s_vStaticLibraries=NULL;
        if( s_vStaticLibraries == NULL ) {
            s_vStaticLibraries = new list< tools::shared_ptr<IkLibrary> >();
        }
    ...
    static IkSolverBasePtr CreateIkSolver(const string& _name, const std::vector<dReal>& vfreeinc, EnvironmentBasePtr penv)
    {
        ...
        boost::mutex::scoped_lock lock(GetLibraryMutex());
        for(list< tools::shared_ptr<IkLibrary> >::reverse_iterator itlib = GetLibraries()->rbegin(); itlib != GetLibraries()->rend(); ++itlib) {
