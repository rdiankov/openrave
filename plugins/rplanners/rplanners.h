...
{
public:
    virtual void Init(tools::weak_ptr<PlannerBase> planner, int dof, boost::function<dReal(const std::vector<dReal>&, const std::vector<dReal>&)>& distmetricfn, dReal fStepLength, dReal maxdistance) = 0;
...
public:
    ...
    virtual void Init(tools::weak_ptr<PlannerBase> planner, int dof, boost::function<dReal(const std::vector<dReal>&, const std::vector<dReal>&)>& distmetricfn, dReal fStepLength, dReal maxdistance)
    {
        Reset();
        if( !!_pNodesPool ) {
    ...
    virtual ExtendType Extend(const vector<dReal>& vTargetConfig, NodeBasePtr& lastnode, bool bOneStep=false)
    {
        ...
        tools::shared_ptr<PlannerBase> planner(_planner);
...
//                }
    ...
    tools::weak_ptr<PlannerBase> _planner;
    dReal _fStepLength;
    int _dof; ///< the number of values of each state
    int _fromgoal;

    // cover tree data structures
    tools::shared_ptr< boost::pool<> > _pNodesPool; ///< pool nodes are created from
