 ...
 */
/// \brief Interpolate a trajectory which starts and stops at the two given waypoints. Path geometry
/// remain the same after the interpolation.
...
/// \brief Interpolate a trajectory connecting (x0Vect, v0Vect) and (x1Vect, v1Vect).
...
/// \brief Given a set of ParabolicCurves with different duration, try to stretch every curve to have the same duration.
...
/// \brief Interpolate a trajectory connecting (x0Vect, v0Vect) and (x1Vect, v1Vect) which has the specified duration
 ...
 */
/// \brief Interpolate a minimum-time 1D trajectory connecting (x0, v0) and (x1, v1).
bool Interpolate1D(dReal x0, dReal x1, dReal v0, dReal v1, dReal vm, dReal am, ParabolicCurve& curveOut);

/// \brief Interpolate a minimum-time 1D trajectory connecting (x0, v0) and (x1, v1), assuming
/// infinite velocity bounds.
bool Interpolate1DNoVelocityLimit(dReal x0, dReal x1, dReal v0, dReal v1, dReal am, ParabolicCurve& curveOut);

/// \brief Given a minimum-time 1D trajectory, impose the given velocity bound on it. If the velocity bound
/// is violated, the modified trajectory will have a longer duration.
bool ImposeVelocityLimit(ParabolicCurve& curve, dReal vm);

/// \brief Given a (not necessarily minimum-time) 1D trajectory, impose the given joint limits on it
...
/// \brief Stretch the given trajectory such that it ends at the given duration. It internally calls Interpolate1DFixedDuration.
bool Stretch1D(ParabolicCurve& curveIn, dReal vm, dReal am, dReal newDuration, ParabolicCurve& curveOut);

/// \brief Interpolate a minimum-acceleration trajectory with the specified duration connecting (x0, v0) and (x1, v1).
