...
#ifndef OPENRAVE_MANIP_CONSTRAINTS2_H
#define OPENRAVE_MANIP_CONSTRAINTS2_H
...
//
    ...
    RampOptimizerInternal::CheckReturn CheckManipConstraints2(const std::vector<RampOptimizerInternal::ParabolicCurvesND> &curvesndVectOut) {
        if (_maxmanipspeed <= 0 && _maxmanipaccel <= 0) {
            return RampOptimizerInternal::CheckReturn(0);
        }

        dReal maxmanipspeed = 0, maxmanipaccel = 0;
        Vector endeffvellin, endeffvelang, endeffacclin, endeffaccang; // end-effector velocities and accelerations (linear and angular)

        FOREACHC(itcurvesnd, curvesndVectOut) {
            if (itcurvesnd->duration <= g_fEpsilonLinear) {
                continue;
            }
            ...
            FOREACHC(itmanipinfo, _listCheckManips) {
                ...
                // Get end-effector velocity and acceleration
                probot->GetLinkVelocities(endeffvels);
                probot->GetLinkAccelerations(_afill, endeffaccs);
                endeffvellin = endeffvels.at(eeIndex).first;
                endeffvelang = endeffvels.at(eeIndex).second;
                endeffacclin = endeffaccs.at(eeIndex).first;
                endeffaccang = endeffaccs.at(eeIndex).second;

                Transform R = itmanipinfo->plink->GetTransform();

                // For each point in checkpoints, compute its velocity and acceleration and check whether they satisfy the manipulator constraints
                FOREACH(itpoint, itmanipinfo->checkpoints) {
                    ...
                    if (_maxmanipspeed > 0) {
                        ...
                        if (manipspeed > 1e5) {
                            RAVELOG_WARN_FORMAT("manipspeed is too large: %.15e", manipspeed);
                        }
                        if (maxmanipspeed < manipspeed) {
                            maxmanipspeed = manipspeed;
                        }
                        if (manipspeed > _maxmanipspeed) {
                            RAVELOG_VERBOSE_FORMAT("maxmanipspeed < manipspeed: %.15e < %.15e", _maxmanipspeed%manipspeed);
                            return RampOptimizerInternal::CheckReturn(CFO_CheckTimeBasedConstraints, 0.9*_maxmanipspeed/manipspeed);
                        }
                    }
                    ...
                    if (_maxmanipaccel > 0) {
                        ...
                        if (manipaccel > _maxmanipaccel) {
                            RAVELOG_VERBOSE_FORMAT("maxmanipaccel < manipaccel: %.15e < %.15e", _maxmanipaccel%manipaccel);
                            return RampOptimizerInternal::CheckReturn(CFO_CheckTimeBasedConstraints, 0.9*_maxmanipaccel/manipaccel);
                        }
                    }
                }
            }
    ...
    RampOptimizerInternal::CheckReturn CheckManipConstraints3(const std::vector<RampOptimizerInternal::ParabolicCurvesND> &curvesndVectOut) {
        if (_maxmanipspeed <= 0 && _maxmanipaccel <= 0) {
            return RampOptimizerInternal::CheckReturn(0);
        }
