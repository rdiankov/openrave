...
public:
    ...
    virtual PlannerStatus PlanPath(TrajectoryBasePtr ptraj, int planningoptions) override
    {
        ...
        if( IS_DEBUGLEVEL(Level_Verbose) ) {
            ...
            f << *_parameters;
            RAVELOG_VERBOSE_FORMAT("env=%d(%s), saved parabolic parameters to %s", GetEnv()->GetId()%GetEnv()->GetName()%filename);
        }
        ...
        if( vusedbodies.size() == 0 ) {
            RAVELOG_WARN_FORMAT("env=%d(%s), there are no used bodies in this configuration", GetEnv()->GetId()%GetEnv()->GetName());
        }
        ...
        if (_parameters->_hastimestamps && itcompatposgroup->interpolation == "quadratic" ) {
            RAVELOG_VERBOSE_FORMAT("env=%d(%s), Initial traj is piecewise quadratic", GetEnv()->GetId()%GetEnv()->GetName());
            ...
            RAVELOG_VERBOSE_FORMAT("env=%d(%s), Initial traj is piecewise cubic", GetEnv()->GetId()%GetEnv()->GetName());
            // assumes that the traj has velocity data and is consistent, so convert the original trajectory in a sequence of ramps, and preserve velocity
            vector<dReal> x0, x1, dx0, dx1, ramptime;
            ptraj->GetWaypoint(0,x0,posspec);
            ptraj->GetWaypoint(0,dx0,velspec);
                ...
                if (ramptime.at(0) > g_fEpsilonLinear) {
                    ...
                    for(size_t j = 0; j < x0.size(); ++j) {
                        ...
                        if( RaveFabs(coeff) > 1e-5 ) {
                            RAVELOG_VERBOSE_FORMAT("env=%d(%s), ramp %d/%d dof %d cubic coeff=%.15e is non-zero", GetEnv()->GetId()%GetEnv()->GetName()%iramp%dynamicpath.ramps.size()%j%coeff);
                            bIsParabolic = false;
                        }
                    }
            ...
            RAVELOG_VERBOSE_FORMAT("env=%d(%s), initial numwaypoints before removing collinear ones = %d", GetEnv()->GetId()%GetEnv()->GetName()%ptraj->GetNumWaypoints());
            ...
            RAVELOG_DEBUG_FORMAT("env=%d(%s), finish initializing the trajectory (via _SetMilestones). #waypoints: %d -> %d", GetEnv()->GetId()%GetEnv()->GetName()%ptraj->GetNumWaypoints()%(dynamicpath.ramps.size() + 1));
        }
        ...
        try {
            _bUsePerturbation = true;
            RAVELOG_DEBUG_FORMAT("env=%d(%s), initial path size=%d, duration=%f, pointtolerance=%f, multidof=%d, manipname=%s, maxmanipspeed=%f, maxmanipaccel=%f, %s", GetEnv()->GetId()%GetEnv()->GetName()%dynamicpath.ramps.size()%dynamicpath.GetTotalTime()%parameters->_pointtolerance%parameters->_multidofinterp%parameters->manipname%parameters->maxmanipspeed%parameters->maxmanipaccel%ssinitial.str());
            ...
            RAVELOG_DEBUG_FORMAT("env=%d(%s), after shortcutting: duration %.15e -> %.15e, diff = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%dummyDur1%dummyDur2%(dummyDur1 - dummyDur2));

#ifdef OPENRAVE_TIMING_DEBUGGING
            RAVELOG_INFO_FORMAT("env=%d(%s), calling checkmanipconstraints %d times, using %.15e sec. = %.15e sec./call", GetEnv()->GetId()%GetEnv()->GetName()%ncheckmanipconstraints%checkmaniptime%(ncheckmanipconstraints == 0 ? 0 : checkmaniptime/ncheckmanipconstraints));
#endif
                    ...
                    if( bCheck ) {
                        ...
                        if( checkret.retcode != 0 ) { // probably don't need to check bDifferentVelocity
                            ...
                            for(size_t idilate = 0; idilate < maxIncrement; ++idilate ) {
                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), ramp %d, idilate=%d/%d", GetEnv()->GetId()%GetEnv()->GetName()%irampindex%idilate%maxIncrement);
                                ...
                                if( ParabolicRamp::SolveAccelBounded(rampndtrimmed.x0, rampndtrimmed.dx0, rampndtrimmed.x1, rampndtrimmed.dx1, endTime,  parameters->_vConfigAccelerationLimit, parameters->_vConfigVelocityLimit, parameters->_vConfigLowerLimit, parameters->_vConfigUpperLimit, tempramps1d, _parameters->_multidofinterp, rampndtrimmed.dx0.size()) ) {
                                    ...
                                    if( temprampsnd[0].endTime > originalEndTime*10 ) {
                                        RAVELOG_WARN_FORMAT("env=%d(%s), new end time=%fs is really big compared to old %fs!", GetEnv()->GetId()%GetEnv()->GetName()%temprampsnd[0].endTime%endTime);
                                        break;
                                    }
                                    ...
                                    RAVELOG_VERBOSE_FORMAT("env=%d(%s), old endtime=%.15e, new endtime=%.15e", GetEnv()->GetId()%GetEnv()->GetName()%rampndtrimmed.endTime%endTime);
                                    FOREACH(itnewrampnd, temprampsnd) {
                                        ...
                                        if( newrampret.retcode != 0 ) {
                                            RAVELOG_VERBOSE_FORMAT("env=%d(%s), has bad ramp retcode=0x%x", GetEnv()->GetId()%GetEnv()->GetName()%newrampret.retcode);
                                            bHasBadRamp = true;
                                            break;
                                        }
                                        else if( newrampret.bDifferentVelocity ) {
                                            RAVELOG_VERBOSE_FORMAT("env=%d(%s), resulting ramp has different final velocity", GetEnv()->GetId()%GetEnv()->GetName());
                                            bHasBadRamp = true;
                                            break;
                                        }
                                        else {
            ...
            RAVELOG_DEBUG_FORMAT("env=%d(%s), after shortcutting %d times: path waypoints=%d, traj waypoints=%d, traj time=%fs", GetEnv()->GetId()%GetEnv()->GetName()%numshortcuts%dynamicpath.ramps.size()%_dummytraj->GetNumWaypoints()%_dummytraj->GetDuration());
        ...
        RAVELOG_DEBUG_FORMAT("env=%d(%s), path optimizing - computation time=%fs", GetEnv()->GetId()%GetEnv()->GetName()%(0.001f*(float)(utils::GetMilliTime()-basetime)));

        if( IS_DEBUGLEVEL(Level_Verbose) || (RaveGetDebugLevel() & Level_VerifyPlans) ) {
            RAVELOG_DEBUG_FORMAT("env=%d(%s), start sampling the trajectory (verification purpose) after shortcutting", GetEnv()->GetId()%GetEnv()->GetName());
            ...
            try {
                ...
                RAVELOG_DEBUG_FORMAT("env=%d(%s), sampling for verification successful", GetEnv()->GetId()%GetEnv()->GetName());
            }
            catch (const std::exception& ex) {
                RAVELOG_WARN_FORMAT("env=%d(%s), sampling for verification failed: %s", GetEnv()->GetId()%GetEnv()->GetName()%ex.what());
                _DumpTrajectory(ptraj, _dumplevel);
            }
        }
        ...
        RAVELOG_INFO_FORMAT("env=%d(%s), shortcutting time=%.15e; iter=%d; time/iter=%.15e", GetEnv()->GetId()%GetEnv()->GetName()%tTotalShortcutTime%_numShortcutIters%(tTotalShortcutTime/_numShortcutIters));
        RAVELOG_INFO_FORMAT("env=%d(%s), measured %d interpolations; total exectime=%.15e; time/iter=%.15e", GetEnv()->GetId()%GetEnv()->GetName()%_nCallsInterpolator%_totalTimeInterpolator%(_totalTimeInterpolator/_nCallsInterpolator));
        RAVELOG_INFO_FORMAT("env=%d(%s), measured %d checkmanips; total exectime=%.15e; time/iter=%.15e", GetEnv()->GetId()%GetEnv()->GetName()%_nCallsCheckManip%_totalTimeCheckManip%(_nCallsCheckManip == 0 ? 0 : _totalTimeCheckManip/_nCallsCheckManip));
        RAVELOG_INFO_FORMAT("env=%d(%s), measured %d checkpathallconstraints; total exectime=%.15e; time/iter=%.15e", GetEnv()->GetId()%GetEnv()->GetName()%_nCallsCheckPathAllConstraints%_totalTimeCheckPathAllConstraints%(_nCallsCheckPathAllConstraints == 0 ? 0 : _totalTimeCheckPathAllConstraints/_nCallsCheckPathAllConstraints));
        RAVELOG_INFO_FORMAT("env=%d(%s), measured %d checkpathallconstraints (in vain); total exectime=%.15e", GetEnv()->GetId()%GetEnv()->GetName()%_nCallsCheckPathAllConstraintsInVain%_totalTimeCheckPathAllConstraintsInVain);
#endif
    ...
    virtual int ConfigFeasible(const ParabolicRamp::Vector& a, const ParabolicRamp::Vector& da, int options)
    {
        ...
        try {
            ...
            RAVELOG_WARN_FORMAT("env=%d(%s), rrtparams path constraints threw an exception: %s", GetEnv()->GetId()%GetEnv()->GetName()%ex.what());
    ...
    virtual ParabolicRamp::CheckReturn SegmentFeasible2(const ParabolicRamp::Vector& a,const ParabolicRamp::Vector& b, const ParabolicRamp::Vector& da,const ParabolicRamp::Vector& db, dReal timeelapsed, int options, std::vector<ParabolicRamp::ParabolicRampND>& outramps)
    {
            ...
            if( _constraintreturn->_configurationtimes.size() > 0 ) {
                    ...
                    if( deltatime > g_fEpsilon ) {
                        ...
                        for(size_t idof = 0; idof < newvel.size(); ++idof) {
                            ...
                            if( RaveFabs(newvel[idof]) > _parameters->_vConfigVelocityLimit.at(idof)+ParabolicRamp::EpsilonV ) {
                                if( 0.9*_parameters->_vConfigVelocityLimit.at(idof) < 0.1*RaveFabs(newvel[idof]) ) {
                                    RAVELOG_WARN_FORMAT("env=%d(%s), new velocity for dof %d is too high %f > %f", GetEnv()->GetId()%GetEnv()->GetName()%idof%newvel[idof]%_parameters->_vConfigVelocityLimit.at(idof));
                                }
                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), retcode = 0x4; idof = %d; newvel[idof] = %.15e; vellimit = %.15e; g_fEpsilon = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%idof%newvel[idof]%_parameters->_vConfigVelocityLimit.at(idof)%ParabolicRamp::EpsilonV);
                        ...
                        for(size_t idof = 0; idof < outramp.ramps.size(); ++idof) {
                            if( outramp.ramps[idof].a1 < -_parameters->_vConfigAccelerationLimit[idof] ) {
                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), idof=%d, a1 changed: %.15e -> %.15e, diff = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%idof%outramp.ramps[idof].a1%(-_parameters->_vConfigAccelerationLimit[idof])%(outramp.ramps[idof].a1 + _parameters->_vConfigAccelerationLimit[idof]));
                            ...
                            if( outramp.ramps[idof].a2 < -_parameters->_vConfigAccelerationLimit[idof] ) {
                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), idof=%d, a2 changed: %.15e -> %.15e, diff = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%idof%outramp.ramps[idof].a2%(-_parameters->_vConfigAccelerationLimit[idof])%(outramp.ramps[idof].a2 + _parameters->_vConfigAccelerationLimit[idof]));
                        ...
                        if( bAccelChanged ) {
                            RAVELOG_VERBOSE_FORMAT("env=%d(%s), deltatime=%.15e", GetEnv()->GetId()%GetEnv()->GetName()%deltatime);
                            if( !outramp.IsValid() ) {
                                RAVELOG_WARN_FORMAT("env=%d(%s), ramp becomes invalid after changing acceleration limits", GetEnv()->GetId()%GetEnv()->GetName());
                ...
                for(size_t idof = 0; idof < curpos.size(); ++idof) {
                    if( RaveFabs(curpos[idof]-b[idof])+g_fEpsilon > ParabolicRamp::EpsilonX ) {
                        RAVELOG_WARN_FORMAT("env=%d(%s), curpos[%d] (%.15e) != b[%d] (%.15e)", GetEnv()->GetId()%GetEnv()->GetName()%idof%curpos[idof]%idof%b[idof]);
        ...
        if( outramps.size() == 0 ) {
            ...
            for(size_t idof = 0; idof < newramp.ramps.size(); ++idof) {
                if( newramp.ramps[idof].a1 < -_parameters->_vConfigAccelerationLimit[idof] ) {
                    RAVELOG_VERBOSE_FORMAT("env=%d(%s), idof=%d, a1 changed: %.15e -> %.15e, diff = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%idof%newramp.ramps[idof].a1%(-_parameters->_vConfigAccelerationLimit[idof])%(newramp.ramps[idof].a1 + _parameters->_vConfigAccelerationLimit[idof]));
                ...
                if( newramp.ramps[idof].a2 < -_parameters->_vConfigAccelerationLimit[idof] ) {
                    RAVELOG_VERBOSE_FORMAT("env=%d(%s), idof=%d, a2 changed: %.15e -> %.15e, diff = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%idof%newramp.ramps[idof].a2%(-_parameters->_vConfigAccelerationLimit[idof])%(newramp.ramps[idof].a2 + _parameters->_vConfigAccelerationLimit[idof]));
        ...
        if( _bmanipconstraints && (options & CFO_CheckTimeBasedConstraints) ) {
            try {
                ...
                RAVELOG_WARN_FORMAT("env=%d(%s), CheckManipConstraints2 (modified=%d) threw an exception: %s", GetEnv()->GetId()%GetEnv()->GetName()%((int)bExpectModifiedConfigurations)%ex.what());
                return 0xffff; // could be anything
            }
        }
...
protected:
    ...
    bool _SetMilestones(std::vector<ParabolicRamp::ParabolicRampND>& ramps, const vector<ParabolicRamp::Vector>& vpath)
    {
        ...
        if(vpath.size()==1) {
            ...
            if(!_parameters->verifyinitialpath) {
                ...
                RAVELOG_VERBOSE_FORMAT("env=%d(%s), Initial path verification is disabled using options=0x%x", GetEnv()->GetId()%GetEnv()->GetName()%options);
            }
            ...
            if( !!_parameters->_neighstatefn ) {
                ...
                while(iwaypoint+1 < vnewpath.size() ) {
                    ...
                    if( _parameters->SetStateValues(xmid) != 0 ) {
                        RAVELOG_WARN_FORMAT("env=%d(%s), could not set values of path %d/%d", GetEnv()->GetId()%GetEnv()->GetName()%iwaypoint%vnewpath.size());
                        return false;
                    }
                    if( _parameters->_neighstatefn(xmid, xmiddelta, NSO_OnlyHardConstraints) == NSS_Failed ) {
                        RAVELOG_WARN_FORMAT("env=%d(%s), failed to get the neighbor of the midpoint of path %d/%d", GetEnv()->GetId()%GetEnv()->GetName()%iwaypoint%vnewpath.size());
                        return false;
                    }
    ...
    bool _ValidateRamp(ParabolicRamp::ParabolicRampND& ramp, int options, int iramp=-1, int numramps=-1)
    {
        ...
        for(; itry < iMaxRetries; ++itry) {
            ...
            if( !res ) {
                ...
                ss << "]";
                RAVELOG_WARN_FORMAT("env=%d(%s), ramp %d/%d SolveMinTimeLinear failed. fCurVelMult=%f; itry=%d; %s", GetEnv()->GetId()%GetEnv()->GetName()%iramp%numramps%fCurVelMult%itry%ss.str());
                return false;
            }
            ...
            if( retseg.retcode == 0 ) {
                ...
                RAVELOG_VERBOSE_FORMAT("env=%d(%s), slowing down ramp %d/%d by %.15e since too fast, try %d", GetEnv()->GetId()%GetEnv()->GetName()%iramp%numramps%retseg.fTimeBasedSurpassMult%itry);
                ...
                SerializeValues(ss, dx1);
                ss << "]; deltatime=" << (vswitchtimes.at(iswitch) - fprevtime);
                RAVELOG_WARN_FORMAT("env=%d(%s), initial ramp starting at %d/%d, switchtime=%f (%d/%d), returned error 0x%x; %s giving up....", GetEnv()->GetId()%GetEnv()->GetName()%iramp%numramps%vswitchtimes.at(iswitch)%iswitch%vswitchtimes.size()%retseg.retcode%ss.str());
                return false;
            }
        }
        if( retseg.retcode != 0 ) {
            ...
            SerializeValues(ss, accellimits);
            ss << "]";
            RAVELOG_WARN_FORMAT("env=%d(%s), ramp %d/%d initialization failed. fCurVelMult=%f; itry=%d; retcode=0x%x; %s", GetEnv()->GetId()%GetEnv()->GetName()%iramp%numramps%fCurVelMult%itry%retseg.retcode%ss.str());
            return false;
        }
        return true;
    }
    ...
    int _Shortcut(ParabolicRamp::DynamicPath& dynamicpath, int numIters, ParabolicRamp::RandomNumberGeneratorBase* rng, dReal mintimestep)
    {
        ...
        for(iters=0; iters<numIters; iters++) {
            ...
            RAVELOG_VERBOSE_FORMAT("env=%d(%s), shortcut iter = %d/%d, shortcutting from t1 = %.15e to t2 = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%t1%t2);
            ...
            try {
                ...
                if( _bmanipconstraints && !!_manipconstraintchecker ) {
                    if( _parameters->SetStateValues(x0) != 0 ) {
                        RAVELOG_VERBOSE_FORMAT("env=%d(%s), state set error", GetEnv()->GetId()%GetEnv()->GetName());
                        continue;
                    }
                ...
                for(islowdowntry = 0; islowdowntry < 4; ++islowdowntry ) {
                    ...
                    if( newramptime+mintimestep > t2-t1 ) {
                        ...
                        RAVELOG_VERBOSE_FORMAT("env=%d(%s), shortcut iter=%d rejected times [%f, %f]. final trajtime=%fs", GetEnv()->GetId()%GetEnv()->GetName()%iters%t1%t2%(endTime-(t2-t1)+newramptime));
                        break;
                    }
                    else {
                        RAVELOG_VERBOSE_FORMAT("env=%d(%s), shortcut iter=%d t1 = %.15e; t2 = %.15e; newramptime = %.15e; tdiff = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%iters%t1%t2%newramptime%(t2-t1));
                    }
                    ...
                    if (_parameters->_nMaxPlanningTime > 0) {
                        ...
                        if( elapsedtime >= _parameters->_nMaxPlanningTime ) {
                            ...
                            RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut time exceeded (%dms) so breaking. iter=%d < %d", GetEnv()->GetId()%GetEnv()->GetName()%elapsedtime%iters%numIters);
                            break;
                        }
                    }
                    ...
                    for(size_t iramp=0; iramp<intermediate.ramps.size(); iramp++) {
                        ...
                        if( retcheck.bDifferentVelocity && outramps.size() > 0 ) {
                            ...
                            if( !res ) {
                                RAVELOG_WARN_FORMAT("env=%d(%s), failed to SolveMinTime for different vel ramp", GetEnv()->GetId()%GetEnv()->GetName());
                                retcheck.retcode = CFO_FinalValuesNotReached;
                                break;
                            }
                            if( RaveFabs(intermediate2.GetTotalTime()-outramp.endTime) > allowedstretchtime) {
                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), intermediate2 ramp duration is too long %fs", GetEnv()->GetId()%GetEnv()->GetName()%intermediate2.GetTotalTime());
                                retcheck.retcode = CFO_FinalValuesNotReached;
                                break;
                            }
                            ...
                            if (retcheck.retcode != 0) {
                                RAVELOG_WARN_FORMAT("env=%d(%s), the final SolveMinTime generated infeasible segment retcode = 0x%x", GetEnv()->GetId()%GetEnv()->GetName()%retcheck.retcode);
                                ...
                                for (size_t idilation = 0; idilation < maxIncrement; ++idilation) {
                                    RAVELOG_VERBOSE_FORMAT("env=%d(%s), idilation = %d/%d", GetEnv()->GetId()%GetEnv()->GetName()%idilation%maxIncrement);
                                    ...
                                    if (res2) {
                                        ...
                                        if (temprampsnd[0].endTime > endTime*10) {
                                            RAVELOG_WARN_FORMAT("env=%d(%s), new endtime (%.15e) is too big compared to prev endtime (%.15e)", GetEnv()->GetId()%GetEnv()->GetName()%temprampsnd[0].endTime%endtime);
                                            break;
                                        }
                                        endtime = temprampsnd[0].endTime;
                                        ...
                                        // Check feasibility
                                        bool bhasbadramp = false;
                                        RAVELOG_VERBOSE_FORMAT("env=%d(%s), original endtime = %.15e; new endtime = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%outramp.endTime%endtime);
                                        FOREACH(itnewrampnd, temprampsnd) {
                                            retcheck = _feasibilitychecker.Check2(*itnewrampnd, 0xffff|CFO_FromTrajectorySmoother, outramps2);
                                            if (retcheck.retcode != 0) {
                                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), has bad ramp: retcode = 0x%x", GetEnv()->GetId()%GetEnv()->GetName()%retcheck.retcode);
                                                bhasbadramp = true;
                                                break;
                                            }
                                            if (retcheck.bDifferentVelocity) {
                                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), still ends with different velocity. terminated", GetEnv()->GetId()%GetEnv()->GetName());
                                                retcheck.retcode = CFO_FinalValuesNotReached;
                                                break;
                                            }
                                        }
                                        if (!bhasbadramp) {
                                            RAVELOG_VERBOSE_FORMAT("env=%d(%s), idilation = %d/%d produces feasible ramp", GetEnv()->GetId()%GetEnv()->GetName()%idilation%maxIncrement);
                                            RAVELOG_VERBOSE_FORMAT("env=%d(%s), time increment = %.15e; allowed stretched time = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%(endtime - outramp.endTime)%allowedstretchtime);
                                            break;
                                        }
                                    }
                                ...
                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), after Check2, intermediate2 does not end at the desired velocity", GetEnv()->GetId()%GetEnv()->GetName());
                                retcheck.retcode = CFO_FinalValuesNotReached;
                                break;
                            }
                            else {
                                // Otherwise, the new segment is good.
                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), the final SolveMinTime generated feasible segment, inserting it to outramps", GetEnv()->GetId()%GetEnv()->GetName());
                                outramps.pop_back();
                            ...
                            RAVELOG_VERBOSE_FORMAT("env=%d(%s), new shortcut is aligned with boundary values after running Check2", GetEnv()->GetId()%GetEnv()->GetName());
                        }
                    ...
                    if( retcheck.retcode == CFO_CheckTimeBasedConstraints ) {
                        RAVELOG_VERBOSE_FORMAT("env=%d(%s), shortcut iter=%d, slow down ramp by fTimeBasedSurpassMult=%.15e, fcurmult=%.15e", GetEnv()->GetId()%GetEnv()->GetName()%iters%retcheck.fTimeBasedSurpassMult%fcurmult);
                        for(size_t j = 0; j < vellimits.size(); ++j) {
                            // have to watch out that velocities don't drop under dx0 & dx1!
                            dReal fminvel = max(RaveFabs(dx0[j]), RaveFabs(dx1[j]));
                        ...
                        if( fcurmult < 0.01 ) {
                            RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter=%d, fcurmult is too small (%.15e) so giving up on this ramp", GetEnv()->GetId()%GetEnv()->GetName()%iters%fcurmult);
                        ...
                        RAVELOG_VERBOSE_FORMAT("env=%d(%s), shortcut iter=%d rejected due to constraints 0x%x", GetEnv()->GetId()%GetEnv()->GetName()%iters%retcheck.retcode);
                        break;
                    }
                    iIterProgress += 0x1000;
                }
                ...
                if( accumoutramps.size() == 0 ) {
                    RAVELOG_WARN_FORMAT("env=%d(%s), accumulated ramps are empty!", GetEnv()->GetId()%GetEnv()->GetName());
                    continue;
                }
                ...
                RAVELOG_WARN_FORMAT("env=%d(%s), exception happened during shortcut iteration progress=0x%x: %s", GetEnv()->GetId()%GetEnv()->GetName()%iIterProgress%ex.what());
        ...
        RAVELOG_INFO_FORMAT("env=%d(%s), shortcutting time = %.15e s.; numiters = %d; avg. time per iteration = %.15e s.", GetEnv()->GetId()%GetEnv()->GetName()%tshortcuttotal%iters%(tshortcuttotal/iters));
        RAVELOG_INFO_FORMAT("env=%d(%s), measured %d slow-down loops, %.15e sec. = %.15e sec./loop", GetEnv()->GetId()%GetEnv()->GetName()%nslowdownloops%slowdownlooptime%(slowdownlooptime/nslowdownloops));
        RAVELOG_INFO_FORMAT("env=%d(%s), measured %d interpolations, %.15e sec. = %.15e sec./interpolation", GetEnv()->GetId()%GetEnv()->GetName()%ninterpolations%interpolationtime%(interpolationtime/ninterpolations));
        RAVELOG_INFO_FORMAT("env=%d(%s), measured %d checkings, %.15e sec. = %.15e sec./check", GetEnv()->GetId()%GetEnv()->GetName()%nchecks%checktime%(checktime/nchecks));
#endif
        ...
        if (IS_DEBUGLEVEL(Level_Verbose)) {
            ...
            RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut progress is written to %s", GetEnv()->GetId()%GetEnv()->GetName()%shortcutprogressfilename);
        }
    ...
    int _Shortcut2(ParabolicRamp::DynamicPath& dynamicpath, int numIters, ParabolicRamp::RandomNumberGeneratorBase *rng, dReal mintimestep) {
        ...
        for (iters = 0; iters < numIters; iters++) {
            ...
            if (nItersFromPrevSuccessful + nNumTimeBasedConstraintsFailed > nCutoffIters) { // the same time based constraints can fail all the time meaning that the trajectory is already pretty optimal. This check makes smoother easier to stop when there's no improvement
                ...
                RAVELOG_DEBUG_FORMAT("env=%d(%s), no progress for shortcutting, so break %d + %d > %d", GetEnv()->GetId()%GetEnv()->GetName()%nItersFromPrevSuccessful%nNumTimeBasedConstraintsFailed%nCutoffIters);
                break;
            }
            ...
            RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d, shortcutting from t1 = %.15e to t2 = %.15e; fstarttimevelmult=%.15e; fstarttimeaccelmult=%.15e", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%t1%t2%fstarttimevelmult%fstarttimeaccelmult);
#endif
            if (t2 - t1 < mintimestep) {
#ifdef SMOOTHER1_PROGRESS_DEBUG
                RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: the sampled t1 and t2 are too close (mintimestep = %.15e)", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%mintimestep);
#endif
                continue;
            }
            {
...
#ifdef SMOOTHER1_PROGRESS_DEBUG
                        RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: the sampled t1 (%f) and t2 (%f) are already tested", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%t1%t2);
#endif
                        continue;
                    }
            ...
            try {
                ...
                if (_parameters->SetStateValues(x0) != 0) {
#ifdef SMOOTHER1_PROGRESS_DEBUG
                    RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: setting state at x0 failed", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters);
#endif
                    continue;
                }
                iIterProgress += 0x10000000;
                ...
                if (_parameters->SetStateValues(x1) != 0) {
#ifdef SMOOTHER1_PROGRESS_DEBUG
                    RAVELOG_VERBOSE_FORMAT("env=%d(%s), shortcut iter = %d/%d: setting state at x1 failed", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters);
#endif
                    continue;
                }
                iIterProgress += 0x10000000;
                ...
                if (0) {
                    if (_parameters->SetStateValues(x0) != 0) {
                        RAVELOG_WARN_FORMAT("env=%d(%s), state setting error", GetEnv()->GetId()%GetEnv()->GetName());
                        break;
                    }
                    _manipconstraintchecker->GetMaxVelocitiesAccelerations(dx0, vellimits, accellimits);
                    if (_parameters->SetStateValues(x1) != 0) {
                ...
                for (islowdowntry = 0; islowdowntry < maxSlowdowns; ++islowdowntry) {
...
#ifdef SMOOTHER1_PROGRESS_DEBUG
                        RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: initial interpolation failed at islowdowntry = %d", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%islowdowntry);
#endif
                        break;
                    }
...
#ifdef SMOOTHER1_PROGRESS_DEBUG
                        RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: new %f (%f+%f) > %f shortcut did not (and will not) make significant improvement", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%(newramptime + mintimestep)%newramptime%mintimestep%(t2 - t1));
#endif
                        break;
                    }
                    ...
                    for (size_t irampnd = 0; irampnd < intermediate.ramps.size(); ++irampnd) {
                        ...
                        if (retcheck.bDifferentVelocity && outramps.size() > 0) {
                            ...
                            if (!res) {
                                RAVELOG_WARN_FORMAT("env=%d(%s), failed to correct velocity discrepancy at the end of the segment", GetEnv()->GetId()%GetEnv()->GetName());
                                retcheck.retcode = CFO_FinalValuesNotReached;
                                break;
                            }
                            if (RaveFabs(intermediate2.GetTotalTime() - outramp.endTime) > allowedstretchtime) {
                                RAVELOG_WARN_FORMAT("env=%d(%s), intermediate2 is too long to be useful", GetEnv()->GetId()%GetEnv()->GetName());
                                retcheck.retcode = CFO_FinalValuesNotReached;
                                break;
                            }
                    ...
                    if (retcheck.retcode == 0) {
                        ...
                        if (_bmanipconstraints && !!_manipconstraintchecker) {
                            ...
                            if (islowdowntryduetomanip == 0 && (retcheck.fMaxManipAccel > _parameters->maxmanipaccel || retcheck.fMaxManipSpeed > _parameters->maxmanipspeed)) {
                                ...
                                if (retcheck.fMaxManipAccel > _parameters->maxmanipaccel) {
...
#ifdef SMOOTHER1_PROGRESS_DEBUG
                                        RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: fcuraccelmult (%.15e) is too small. continue to the next iteration", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%fcuraccelmult);
#endif
                                        break;
                                    }
                                    {
                                        ...
                                        if (fcurvelmult < 0.01) {
#ifdef SMOOTHER1_PROGRESS_DEBUG
                                            RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: fcurvelmult (%.15e) is too small. continue to the next iteration", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%fcurvelmult);
#endif
                                            break;
                                        }
                                    ...
                                    if (fcuraccelmult < 0.0001) {
#ifdef SMOOTHER1_PROGRESS_DEBUG
                                        RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: faccelmult (%.15e) is too small. continue to the next iteration", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%fcuraccelmult);
#endif
                                        break;
                                    }
                                    for (size_t j = 0; j < accellimits.size(); ++j) {
                                ...
                                numslowdowns += 1;
                                RAVELOG_VERBOSE_FORMAT("env=%d(%s), fTimeBasedSurpassMult = %.15e; fcurvelmult = %.15e; fcuraccelmult = %.15e", GetEnv()->GetId()%GetEnv()->GetName()%retcheck.fTimeBasedSurpassMult%fcurvelmult%fcuraccelmult);
                            }
                        }
                        else {
                        ...
                        if (expectedRampTimeAfterSlowDown + mintimestep > t2 - t1) {
                            // Reject this shortcut since it did not (and will not) make any significant improvement.
#ifdef SMOOTHER1_PROGRESS_DEBUG
                            RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: new %f (%f+%f) > %f shortcut did not (and will not) make significant improvement", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%(expectedRampTimeAfterSlowDown + mintimestep)%expectedRampTimeAfterSlowDown%mintimestep%(t2 - t1));
#endif
                            break;
                        }
                    }
...
#ifdef SMOOTHER1_PROGRESS_DEBUG
                        RAVELOG_DEBUG_FORMAT("env=%d(%s), shortcut iter = %d/%d: shortcut rejected due to constraints 0x%x", GetEnv()->GetId()%GetEnv()->GetName()%iters%numIters%retcheck.retcode);
#endif
                        break;
                    }
                    iIterProgress += 0x1000;
                ...
                RAVELOG_WARN_FORMAT("env=%d(%s), exception happened during shortcut iterprogress = 0x%x: %s", GetEnv()->GetId()%GetEnv()->GetName()%iIterProgress%ex.what());
            }
        }
#ifdef OPENRAVE_TIMING_DEBUGGING
        uint32_t tshortcutend = utils::GetMicroTime();
