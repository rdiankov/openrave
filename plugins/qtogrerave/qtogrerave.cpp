...
OpenRAVE::ViewerBasePtr CreateQtOgreViewer(OpenRAVE::EnvironmentBasePtr penv, std::istream& sinput);
}; // namespace qtogrerave

// for some reason windows complains when the prototypes are different
OpenRAVE::InterfaceBasePtr CreateInterfaceValidated(OpenRAVE::InterfaceType type, const std::string& interfacename, std::istream& sinput, EnvironmentBasePtr penv)
{
    //  Debug.
    RAVELOG_DEBUG("Initiating QtOgreRave plugin...!!!!.\n");
...
#endif
        if( interfacename == "qtogre" ) {
            // not sure if should be creating application all the time...

            //if( QApplication::startingUp() ) {
            QCoreApplication *qapp = QGuiApplication::instance();
            if( !qapp ) {
                RAVELOG_VERBOSE("qt is not initialized yet, so initializing...\n");
                static int s_QtArgc = 0; // has to be static!
                qapp = new QGuiApplication(s_QtArgc,NULL);
                return qtogrerave::CreateQtOgreViewer(penv, sinput);
            } else {
                return qtogrerave::CreateQtOgreViewer(penv, sinput); // no idea...
...
{
    info.interfacenames[PT_Viewer].push_back("qtogre");
}
