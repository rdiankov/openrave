...
def customcse(rawexprs,symbols=None):
    ...
    return newreplacements,reduced_exprs, checks
...
"""
    ...
    def generateChain(self, node):
        ...
        if node.Tfk:
            ...
            subexprs,reduced_exprs, greaterzerochecks = customcse (node.Tfk[0:3,0:4].subs([(v[0],Symbol('j[%d]'%v[1])) for v in allvars]),self.symbolgen)
    ...
    def generateIKChainRotation3D(self, node):
        ...
        if node.Rfk:
            code += self.getFKFunctionPreamble()
            allvars = node.solvejointvars + node.freejointvars
            subexprs,reduced_exprs,greaterzerochecks = customcse(node.Rfk[0:3,0:3].subs([(v[0],Symbol('j[%d]'%v[1])) for v in allvars]),self.symbolgen)
    ...
    def generateIKChainTranslation3D(self, node):
        ...
        if node.Pfk:
            ...
            subexprs,reduced_exprs,greaterzerochecks = customcse(eqs,self.symbolgen)
...
#             self.WriteEquations2(lambda k: var,value,code=code)
    ...
    def _WriteDictEquations(self, dictequations, code=None):
        ...
        replacements,reduced_exprs,greaterzerochecks = customcse(exprs,symbols=self.symbolgen)
