...
public:
        ...
        PyIkSolverBasePtr GetIkSolver() {
            IkSolverBasePtr iksolver = _pmanip->GetIkSolver(); return !iksolver ? PyIkSolverBasePtr() : PyIkSolverBasePtr(new PyIkSolverBase(iksolver,_pyenv));
        }
...
public:
        ...
        PySensorBasePtr GetSensor() {
            return !_pattached->GetSensor() ? PySensorBasePtr() : PySensorBasePtr(new PySensorBase(_pattached->GetSensor(),_pyenv));
        }
...
public:
    ...
    PyControllerBasePtr GetController() const {
        return !_probot->GetController() ? PyControllerBasePtr() : PyControllerBasePtr(new PyControllerBase(_probot->GetController(),_pyenv));
    }
...
public:
    ...
    bool InitKinBody(PyKinBodyPtr pbody) {
        CHECK_POINTER(pbody); return _pPhysicsEngine->InitKinBody(pbody->GetBody());
    }
...
public:
    ...
    void AddKinBody(PyKinBodyPtr pbody) {
        CHECK_POINTER(pbody); _penv->AddKinBody(pbody->GetBody());
    }
    ...
    void AddSensor(PySensorBasePtr sensor, bool bAnonymous) {
        CHECK_POINTER(sensor); _penv->AddSensor(sensor->GetSensor(),bAnonymous);
    }
    void AddViewer(PyViewerBasePtr viewer) {
        CHECK_POINTER(viewer); _penv->AddViewer(viewer->GetViewer());
    }

    bool RemoveKinBody(PyKinBodyPtr pbody) {
        CHECK_POINTER(pbody); RAVELOG_WARN("openravepy RemoveKinBody deprecated, use Remove\n"); return _penv->Remove(pbody->GetBody());
    }
    ...
    int AddModule(PyModuleBasePtr prob, const string& args) {
        CHECK_POINTER(prob); return _penv->AddModule(prob->GetModule(),args);
    }
    bool RemoveProblem(PyModuleBasePtr prob) {
        CHECK_POINTER(prob); RAVELOG_WARN("openravepy RemoveProblem deprecated, use Remove\n"); return _penv->Remove(prob->GetModule());
    }
    bool Remove(PyInterfaceBasePtr obj) {
        CHECK_POINTER(obj); return _penv->Remove(obj->GetInterfaceBase());
    }
    ...
    PyPhysicsEngineBasePtr GetPhysicsEngine() {
        return PyPhysicsEngineBasePtr(new PyPhysicsEngineBase(_penv->GetPhysicsEngine(),shared_from_this()));
    }
    ...
    {
        ViewerBasePtr pv = _penv->GetViewer();
        return !pv ? PyViewerBasePtr() : PyViewerBasePtr(new PyViewerBase(_penv->GetViewer(),shared_from_this()));
    }
