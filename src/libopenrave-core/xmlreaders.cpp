...
public:
    ...
    virtual ProcessElement startElement(const std::string& xmlname, const AttributesList& atts)
    {
                ...
                if( xmlname == "render" ) {
                    ...
                    FOREACHC(itatt,atts) {
                        if( itatt->first == "file" ) {
                            ...
                            _renderfilename.second = vscale*_fScaleGeometry;
                        }
                    }
                }
                    ...
                    FOREACHC(itatt,atts) {
                        if( itatt->first == "file" ) {
                            ...
                            sslocal >> vscale.x; vscale.y = vscale.z = vscale.x;
                            sslocal >> vscale.y >> vscale.z;
                            _collisionfilename.second = vscale*_fScaleGeometry;
                        }
                    }
                }
        ...
        if( xmlname == "body" ) {
            ...
            newatts.push_back(make_pair("scalegeometry",str(boost::format("%f")%_fScaleGeometry)));
    ...
    virtual bool endElement(const std::string& xmlname)
    {
        if( !!_pcurreader ) {
            ...
            if( xmlname == "translation" ) {
                Vector v;
                _ss >>v.x >> v.y >> v.z;
                _itgeomprop->_t.trans += v*_fScaleGeometry;
            }
                ...
                case KinBody::Link::GEOMPROPERTIES::GeomCylinder:
                    if( xmlname == "radius") {
                        ...
                        _itgeomprop->vGeomData.y *= _fScaleGeometry;
                    }
                    break;
                case KinBody::Link::GEOMPROPERTIES::GeomTrimesh:
        ...
        if( xmlname == "translation" ) {
            Vector v;
            _ss >> v.x >> v.y >> v.z;
            _plink->_t.trans += v*_fScaleGeometry;
        }
        else if( xmlname == "rotationmat" ) {
            TransformMatrix tnew;
            _ss >> tnew.m[0] >> tnew.m[1] >> tnew.m[2] >> tnew.m[4] >> tnew.m[5] >> tnew.m[6] >> tnew.m[8] >> tnew.m[9] >> tnew.m[10];
    ...
    virtual bool endElement(const std::string& xmlname)
    {
        if( !!_pcurreader ) {
            ...
            if( xmlname == "translation" ) {
                Vector v;
                _ss >> v.x >> v.y >> v.z;
                _trans.trans += v*_fScaleGeometry;
            }
            else if( xmlname == "rotationaxis" ) {
                Vector vaxis; dReal fangle=0;
                _ss >> vaxis.x >> vaxis.y >> vaxis.z >> fangle;
...
public:
    ...
    virtual bool endElement(const std::string& xmlname)
    {
        ...
        if( xmlname == "manipulator" ) {
            ...
            Vector v;
            _ss >> v.x >> v.y >> v.z;
            _pmanip->_tGrasp.trans += v*_fScaleGeometry;
        }
        else if( xmlname == "quat" ) {
            Transform tnew;
            _ss >> tnew.rot.x >> tnew.rot.y >> tnew.rot.z >> tnew.rot.w;
...
public:
    ...
    virtual bool endElement(const std::string& xmlname)
    {
        if( !!_pcurreader ) {
            ...
            Vector v;
            _ss >> v.x >> v.y >> v.z;
            _psensor->trelative.trans += v*_fScaleGeometry;
        }
        else if( xmlname == "quat" ) {
            Transform tnew;
            _ss >> tnew.rot.x >> tnew.rot.y >> tnew.rot.z >> tnew.rot.w;
    ...
    virtual bool endElement(const std::string& xmlname)
    {
        if( !!_pcurreader ) {
            ...
            if( xmlname == "translation" ) {
                Vector v;
                _ss >> v.x >> v.y >> v.z;
                _tsensor.trans += v*_fScaleGeometry;
            }
            else if( xmlname == "quat" ) {
                Transform tnew;
                _ss >> tnew.rot.x >> tnew.rot.y >> tnew.rot.z >> tnew.rot.w;
