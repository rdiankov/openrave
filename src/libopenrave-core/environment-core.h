...
public:
    ...
    typedef tools::shared_ptr<GraphHandleMulti> GraphHandleMultiPtr;

    class CollisionCallbackData : public UserData
    {
public:
        CollisionCallbackData(const CollisionCallbackFn& callback, tools::shared_ptr<Environment> penv) : _callback(callback), _pweakenv(penv) {
        }
        virtual ~CollisionCallbackData() {
            tools::shared_ptr<Environment> penv = _pweakenv.lock();
            if( !!penv ) {
...
protected:
        tools::weak_ptr<Environment> _pweakenv;
    };
    ...
    {
public:
        BodyCallbackData(const BodyCallbackFn& callback, tools::shared_ptr<Environment> penv) : _callback(callback), _pweakenv(penv) {
        }
...
protected:
    ...
    typedef tools::shared_ptr<BodyCallbackData> BodyCallbackDataPtr;
...
public:
    ...
    {
        EnvironmentMutex::scoped_lock lockenv(GetMutex());
        tools::shared_ptr<Environment> penv(new Environment());
        penv->_Clone(tools::static_pointer_cast<Environment const>(shared_from_this()),options,false);
        return penv;
    }
    ...
    {
        EnvironmentMutex::scoped_lock lockenv(GetMutex());
        _Clone(tools::static_pointer_cast<Environment const>(preference),cloningoptions,true);
    }
    ...
    {
        boost::timed_mutex::scoped_lock lock(_mutexInterfaces);
        BodyCallbackDataPtr pdata(new BodyCallbackData(callback,tools::dynamic_pointer_cast<Environment>(shared_from_this())));
    ...
    {
        boost::timed_mutex::scoped_lock lock(_mutexInterfaces);
        CollisionCallbackDataPtr pdata(new CollisionCallbackData(callback,tools::dynamic_pointer_cast<Environment>(shared_from_this())));
    ...
    virtual void GetRegisteredCollisionCallbacks(std::list<CollisionCallbackFn>& listcallbacks) const
    {
        ...
        FOREACHC(it, _listRegisteredCollisionCallbacks) {
            CollisionCallbackDataPtr pdata = tools::dynamic_pointer_cast<CollisionCallbackData>(it->lock());
...
//                }
    ...
    virtual InterfaceBasePtr ReadInterfaceURI(const std::string& filename, const AttributesList& atts)
    {
        try {
            ...
            bool bSuccess = _ParseXMLFile(preader, filename);
            tools::shared_ptr<OpenRAVEXMLParser::InterfaceXMLReadable> preadable = tools::dynamic_pointer_cast<OpenRAVEXMLParser::InterfaceXMLReadable>(preader->GetReadable());
    ...
    virtual tools::shared_ptr<TriMesh> ReadTrimeshURI(tools::shared_ptr<TriMesh> ptrimesh, const std::string& filename, const AttributesList& atts)
    {
        RaveVector<float> diffuseColor, ambientColor;
        return _ReadTrimeshURI(ptrimesh,filename,diffuseColor, ambientColor, atts);
    }

    virtual tools::shared_ptr<TriMesh> _ReadTrimeshURI(tools::shared_ptr<TriMesh> ptrimesh, const std::string& filename, RaveVector<float>& diffuseColor, RaveVector<float>& ambientColor, const AttributesList& atts)
    {
        //EnvironmentMutex::scoped_lock lockenv(GetMutex()); // don't lock!
        string filedata = RaveFindLocalFile(filename);
        if( filedata.size() == 0 ) {
            return tools::shared_ptr<TriMesh>();
        }
    ...
    virtual tools::shared_ptr<TriMesh> ReadTrimeshData(tools::shared_ptr<TriMesh> ptrimesh, const std::string& data, const std::string& formathint, const AttributesList& atts)
    {
        RaveVector<float> diffuseColor, ambientColor;
        return _ReadTrimeshData(ptrimesh, data, formathint, diffuseColor, ambientColor, atts);
    }

    virtual tools::shared_ptr<TriMesh> _ReadTrimeshData(tools::shared_ptr<TriMesh> ptrimesh, const std::string& data, const std::string& formathint, RaveVector<float>& diffuseColor, RaveVector<float>& ambientColor, const AttributesList& atts)
    {
        if( data.size() == 0 ) {
            return tools::shared_ptr<TriMesh>();
        }
...
protected:
    ...
    virtual void _Clone(tools::shared_ptr<Environment const> r, int options, bool bCheckSharedResources=false)
    {
    ...
    void _StartSimulationThread()
    {
        if( !_threadSimulation ) {
            ...
            _threadSimulation.reset(new boost::thread(tools::bind(&Environment::_SimulationThread, this)));
        }
    }
    ...
    void _SimulationThread()
    {
        ...
        while( _bInit && !_bShutdownSimulation ) {
            ...
            tools::shared_ptr<EnvironmentMutex::scoped_try_lock> lockenv;
            if( _bEnableSimulation ) {
                bNeedSleep = false;
    ...
    void _CallBodyCallbacks(KinBodyPtr pbody, int action)
    {
        ...
        FOREACH(it, listRegisteredBodyCallbacks) {
            BodyCallbackDataPtr pdata = tools::dynamic_pointer_cast<BodyCallbackData>(it->lock());
            if( !!pdata ) {
    ...
    tools::shared_ptr<EnvironmentMutex::scoped_try_lock> _LockEnvironmentWithTimeout(uint64_t timeout)
    {
        // try to acquire the lock
#if BOOST_VERSION >= 103500
        tools::shared_ptr<EnvironmentMutex::scoped_try_lock> lockenv(new EnvironmentMutex::scoped_try_lock(GetMutex(),boost::defer_lock_t()));
#else
        tools::shared_ptr<EnvironmentMutex::scoped_try_lock> lockenv(new EnvironmentMutex::scoped_try_lock(GetMutex(),false));
#endif
    ...
    tools::shared_ptr<boost::thread> _threadSimulation;                      ///< main loop for environment simulation
