...
public:
    ...
    typedef tools::shared_ptr<InterfaceType> InterfaceTypePtr;
...
public:
        ...
        domNodeRef _node;
        tools::shared_ptr<daeURI> _nodeurifromphysics; ///< node URL from instance_rigid_body
        domLinkRef _domlink;
...
public:
    ...
    bool Extract()
    {
        ...
        FOREACH(itsensor, vsensors) {
            if( (*itsensor)->Supports(SensorBase::ST_Camera) ) {
                SensorBase::CameraGeomDataConstPtr pcamgeom = tools::static_pointer_cast<SensorBase::CameraGeomData const>((*itsensor)->GetSensorGeometry(SensorBase::ST_Camera));
                if( pcamgeom->target_region.size() > 0 ) {
                    ...
                    FOREACHC(ittestbody, vbodies) {
                        ColladaXMLReadablePtr pcolladainfo = tools::dynamic_pointer_cast<ColladaXMLReadable>((*ittestbody)->GetReadableInterface(ColladaXMLReadable::GetXMLIdStatic()));
                        if( !!pcolladainfo ) {
    ...
    {
        std::list< pair<domInstance_kinematics_modelRef, tools::shared_ptr<KinematicsSceneBindings> > > listPossibleBodies;
        domCOLLADA::domSceneRef allscene = _dom->getScene();
        if( !allscene ) {
            return false;
        }
        ...
        for (size_t iscene = 0; iscene < allscene->getInstance_kinematics_scene_array().getCount(); iscene++) {
            ...
            tools::shared_ptr<KinematicsSceneBindings> bindings(new KinematicsSceneBindings());
            _ExtractKinematicsVisualBindings(allscene->getInstance_visual_scene(),kiscene,*bindings);
            _ExtractPhysicsBindings(allscene,*bindings);
            for(size_t ias = 0; ias < kscene->getInstance_articulated_system_array().getCount(); ++ias) {
        ...
        if (!pdomlink) {
            ...
            for (size_t iatt = 0; iatt < pdomlink->getAttachment_full_array().getCount(); ++iatt) {
                ...
                for (size_t ic = 0; ic < vdomaxes.getCount(); ++ic) {
                    ...
                    tools::shared_ptr<uint8_t> is_circular;
...
//                continue;
            ...
            bindings.listAxisBindings.push_back(JointAxisBinding(tools::bind(&ColladaReader::_InstantiateNode, this, std::placeholders::_1), pjtarget, pjointaxis, jointvalue, NULL, NULL, listInstanceScope));
        }
    }
                    ...
                    FOREACH(itlinkgeomgroups, mapGeometryGroups) {
                        FOREACH(itgeomgroup, itlinkgeomgroups->second) {
                            ...
                            FOREACH(itgeominfo, itgeomgroup->second) {
                                vgeometries.push_back(tools::make_shared<KinBody::GeometryInfo>(*itgeominfo));
                            }
