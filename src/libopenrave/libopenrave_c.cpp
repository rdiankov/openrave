...
typedef std::map<EnvironmentBasePtr, tools::shared_ptr<boost::thread> > VIEWERMAP;
...
bool ORCEnvironmentSetViewer(void* env, const char* viewername)
{
    ...
    if( !!viewername && strlen(viewername) > 0 ) {
        boost::mutex::scoped_lock lock(s_mutexViewer);
        tools::shared_ptr<boost::thread> threadviewer(new boost::thread(tools::bind(CViewerThread, penv, std::string(viewername), true)));
