...
bool KinBody::CheckLinkCollision(int ilinkindex, const Transform& tlinktrans, KinBodyConstPtr pbody, CollisionReportPtr report)
{
    ...
    if( plink->IsEnabled() ) {
        ...
        GetEnv()->GetCollisionCheckers(vCheckers);
        for(CollisionCheckerBasePtr pCollisionChecker : vCheckers) {
            if( pCollisionChecker->CheckCollision(LinkConstPtr(plink),pbody,report) ) {
                if( !bAllLinkCollisions ) { // if checking all collisions, have to continue
                    return true;
                }
                bincollision = true;
...
bool KinBody::CheckLinkCollision(int ilinkindex, const Transform& tlinktrans, CollisionReportPtr report)
{
    ...
    if( plink->IsEnabled() ) {
        ...
        GetEnv()->GetCollisionCheckers(vCheckers);
        for(CollisionCheckerBasePtr pCollisionChecker : vCheckers) {
            if( pCollisionChecker->CheckCollision(LinkConstPtr(plink),report) ) {
                if( !bAllLinkCollisions ) { // if checking all collisions, have to continue
                    return true;
                }
                bincollision = true;
