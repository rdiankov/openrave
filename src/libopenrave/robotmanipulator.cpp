...
RobotBase::Manipulator::Manipulator(RobotBasePtr probot, OPENRAVE_SHARED_PTR<RobotBase::Manipulator const> r)
{
    *this = *r.get();
    __probot = probot;
    if( !!r->GetBase() ) {
...
bool RobotBase::Manipulator::FindIKSolution(const IkParameterization& goal, const std::vector<dReal>& vFreeParameters, vector<dReal>& solution, int filteroptions) const
{
    ...
    OPENRAVE_SHARED_PTR< vector<dReal> > psolution(&solution, utils::null_deleter());
...
bool RobotBase::Manipulator::CheckEndEffectorSelfCollision(CollisionReportPtr report, bool bIgnoreManipulatorLinks) const
{
    ...
    if( bIgnoreManipulatorLinks ) {
        ...
        FOREACHC(itlink,vattachedlinks) {
            ...
            if( plink->IsEnabled() ) {
                OPENRAVE_SHARED_PTR<TransformSaver<LinkPtr> > linksaver(new TransformSaver<LinkPtr>(plink)); // gcc optimization bug when linksaver is on stack?
    ...
    FOREACHC(itlink, probot->GetLinks()) {
        ...
        for(size_t ijoint = 0; ijoint < probot->GetJoints().size(); ++ijoint) {
            if( probot->DoesAffect(ijoint,ilink) && !probot->DoesAffect(ijoint,iattlink) ) {
                bIsAffected = true;
                if( bIgnoreManipulatorLinks ) {
                    OPENRAVE_SHARED_PTR<TransformSaver<LinkPtr> > linksaver(new TransformSaver<LinkPtr>(*itlink)); // gcc optimization bug when linksaver is on stack?
