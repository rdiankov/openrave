...
//#define INTERFACE_PREDELETER std::bind(&RaveDatabase::_InterfaceDestroyCallbackShared,shared_from_this(),_1)
...
public:
    ...
    {
public:
        Plugin(std::shared_ptr<RaveDatabase> pdatabase) : _pdatabase(pdatabase), plibrary(NULL), pfnCreate(NULL), pfnCreateNew(NULL), pfnGetPluginAttributes(NULL), pfnGetPluginAttributesNew(NULL), pfnDestroyPlugin(NULL), _bShutdown(false), _bInitializing(true) {
        }
        virtual ~Plugin() {
            Destroy();
        }
