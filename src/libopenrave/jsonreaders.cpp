...
StringJSONReadable::StringJSONReadable(const std::string& xmlid, const std::string& data) : Readable(xmlid), _data(data)
{
}

void StringJSONReadable::Serialize(BaseJSONWriterPtr writer, int options) const
{
    if( writer->GetFormat() == "collada" ) {
        ...
        BaseJSONWriterPtr child = writer->AddChild("extra",atts);
        atts.clear();
...
const std::string& StringJSONReadable::GetData() const
{
    return _data;
}

HierarchicalJSONReadable::HierarchicalJSONReadable(const std::string& xmlid, const AttributesList& atts) : Readable(xmlid), _atts(atts)
{
}

void HierarchicalJSONReadable::Serialize(BaseJSONWriterPtr writer, int options) const
{
    writer->SetCharData(_data);
    for(std::list<HierarchicalJSONReadablePtr>::const_iterator it = _listchildren.begin(); it != _listchildren.end(); ++it) {
        BaseJSONWriterPtr childwriter = writer->AddChild((*it)->GetXMLId(), (*it)->_atts);
...
BaseJSONReader::ProcessElement TrajectoryReader::startElement(const std::string& name, const AttributesList& atts)
{
    _ss.str("");
    if( !!_pcurreader ) {
    ...
    if( _bInReadable ) {
        _pcurreader.reset(new HierarchicalJSONReader(name,atts));
        return PE_Support;
    }
    if( name == "trajectory" ) {
        ...
        _pcurreader.reset(new ConfigurationSpecification::JSONReader(_spec));
        return PE_Support;
    }
...
bool TrajectoryReader::endElement(const std::string& name)
{
    if( !!_pcurreader ) {
        if( _pcurreader->endElement(name) ) {
            if( _bInReadable ) {
                HierarchicalJSONReaderPtr reader = boost::dynamic_pointer_cast<HierarchicalJSONReader>(_pcurreader);
...
BaseJSONReader::ProcessElement GeometryInfoReader::startElement(const std::string& xmlname, const AttributesList& atts)
{
    _ss.str("");
    if( !!_pcurreader ) {
...
bool GeometryInfoReader::endElement(const std::string& xmlname)
{
    ...
    if( xmlname == _pgeom->GetXMLId() || xmlname == "geom" ) {
        ...
        case GT_TriMesh:
            ...
            break;
        default:
            _pcurreader.reset(new DummyJSONReader(xmlname,"geom"));
        }
    }
...
BaseJSONReader::ProcessElement ElectricMotorActuatorInfoReader::startElement(const std::string& xmlname, const AttributesList& atts)
{
    _ss.str("");
    if( !!_pcurreader ) {
        if( _pcurreader->startElement(xmlname, atts) == PE_Support ) {
...
HierarchicalJSONReader::HierarchicalJSONReader(const std::string& xmlid, const AttributesList& atts) : _xmlid(xmlid)
{
    _readable.reset(new HierarchicalJSONReadable(xmlid,atts));
}

BaseJSONReader::ProcessElement HierarchicalJSONReader::startElement(const std::string& name, const AttributesList& atts)
{
    if( !!_pcurreader ) {
        if( _pcurreader->startElement(name, atts) == PE_Support ) {
            return PE_Support;
...
bool HierarchicalJSONReader::endElement(const std::string& name)
{
    if( !!_pcurreader ) {
        if( _pcurreader->endElement(name) ) {
...
void HierarchicalJSONReader::characters(const std::string& ch)
{
    if( !_pcurreader ) {
        _readable->_data += ch;
    }
...
ReadablePtr HierarchicalJSONReader::GetReadable()
{
    return _readable;
}


StreamJSONWriter::StreamJSONWriter(const std::string& xmltag, const AttributesList& atts) : _xmltag(xmltag), _atts(atts)
{
}

const std::string& StreamJSONWriter::GetFormat() const
{
    static const std::string format("json");
    return format;
}

void StreamJSONWriter::SetCharData(const std::string& data)
{
    _data = data;
}

BaseJSONWriterPtr StreamJSONWriter::AddChild(const std::string& xmltag, const AttributesList& atts)
{
    boost::shared_ptr<StreamJSONWriter> child(new StreamJSONWriter(xmltag,atts));
    _listchildren.push_back(child);
    return child;
}

void StreamJSONWriter::Serialize(std::ostream& stream)
{
    if( _xmltag.size() > 0 ) {
        stream << "<" << _xmltag << " ";
        FOREACHC(it, _atts) {
