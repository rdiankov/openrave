...
std::shared_ptr<ConfigurationSpecification::SetConfigurationStateFn> ConfigurationSpecification::GetSetFn(EnvironmentBasePtr penv) const
{
    ...
    for(size_t igroup = 0; igroup < _vgroups.size(); ++igroup) {
        ...
        if( g.name.size() >= 12 && g.name.substr(0,12) == "joint_values" ) {
            ...
            setstatefns[isavegroup].first = std::bind(SetDOFValuesIndicesParameters, pbody, _1, dofindices, _2);
            ...
            setstatefns[isavegroup].first = std::bind(SetDOFVelocitiesIndicesParameters, pbody, _1, dofindices, _2);
            setstatefns[isavegroup].second = g.dof;
        }
            ...
            setstatefns[isavegroup].first = std::bind(SetBodyTransformFromAffineDOFValues, _1, pbody, affinedofs, vaxis, _2);
            setstatefns[isavegroup].second = g.dof;
        }
            ...
            setstatefns[isavegroup].first = std::bind(SetBodyVelocityFromAffineDOFVelocities, _1, pbody, affinedofs, vaxis, _2);
            setstatefns[isavegroup].second = g.dof;
        }
...
//        }
    ...
    fn.reset(new SetConfigurationStateFn(std::bind(CallSetStateValuesFns, setstatefns, GetDOF(), nMaxDOFForGroup, _1,0)));
    return fn;
}
...
std::shared_ptr<ConfigurationSpecification::GetConfigurationStateFn> ConfigurationSpecification::GetGetFn(EnvironmentBasePtr penv) const
{
    ...
    for(size_t igroup = 0; igroup < _vgroups.size(); ++igroup) {
        ...
        if( g.name.size() >= 12 && g.name.substr(0,12) == "joint_values" ) {
            ...
            getstatefns[isavegroup].first = std::bind(getdofvaluesptr, pbody, _1, dofindices);
            ...
            getstatefns[isavegroup].first = std::bind(getdofvelocitiesptr, pbody, _1, dofindices);
            getstatefns[isavegroup].second = g.dof;
        }
        else if( g.name.size() >= 16 && g.name.substr(0,16) == "affine_transform" ) {
            ss.clear(); ss.str(g.name.substr(12));
            ...
            getstatefns[isavegroup].first = std::bind(GetAffineDOFValuesFromBodyTransform, _1, pbody, affinedofs, vaxis);
            getstatefns[isavegroup].second = g.dof;
        }
        else if( g.name.size() >= 17 && g.name.substr(0,17) == "affine_velocities" ) {
            ss.clear(); ss.str(g.name.substr(12));
            ...
            getstatefns[isavegroup].first = std::bind(GetAffineDOFVelocitiesFromBodyVelocity, _1, pbody, affinedofs, vaxis);
            getstatefns[isavegroup].second = g.dof;
        }
//        else if( g.name.size() >= 4 && g.name.substr(0,4) == "grabbody" ) {
//        }
    ...
    fn.reset(new GetConfigurationStateFn(std::bind(CallGetStateFns,getstatefns, GetDOF(), nMaxDOFForGroup, _1)));
    return fn;
}
                ...
                if( (uninitdata & DOF_RotationMask) && (affinetarget & DOF_RotationMask) && (affinesource & DOF_RotationMask) ) {
                    ...
                    if( affinetarget & DOF_RotationAxis ) {
                        if( affinesource & DOF_Rotation3D ) {
                            rotconverterfn = std::bind(ConvertDOFRotation_AxisFrom3D,_1,_2,targetaxis);
                        }
                        else if( affinesource & DOF_RotationQuat ) {
                            rotconverterfn = std::bind(ConvertDOFRotation_AxisFromQuat,_1,_2,targetaxis);
                        }
                    }
                    else if( affinetarget & DOF_Rotation3D ) {
                        if( affinesource & DOF_RotationAxis ) {
                            rotconverterfn = std::bind(ConvertDOFRotation_3DFromAxis,_1,_2,sourceaxis);
                        }
                        else if( affinesource & DOF_RotationQuat ) {
                        ...
                        if( affinesource & DOF_RotationAxis ) {
                            rotconverterfn = std::bind(ConvertDOFRotation_QuatFromAxis,_1,_2,sourceaxis);
                        }
