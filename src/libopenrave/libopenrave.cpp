...
class RaveGlobal : private boost::noncopyable, public tools::enable_shared_from_this<RaveGlobal>, public UserData
{
...
public:
    ...
    static tools::shared_ptr<RaveGlobal>& instance()
    {
    ...
    {
public:
        XMLReaderFunctionData(InterfaceType type, const std::string& xmltag, const CreateXMLReaderFn& fn, tools::shared_ptr<RaveGlobal> global) : _global(global), _type(type), _xmltag(xmltag)
        {
        ...
        {
            tools::shared_ptr<RaveGlobal> global = _global.lock();
            if( !!global ) {
                boost::mutex::scoped_lock lock(global->_mutexinternal);
...
protected:
        ...
        tools::weak_ptr<RaveGlobal> _global;
        InterfaceType _type;
        std::string _xmltag;
    };
    ...
    tools::shared_ptr<RaveDatabase> GetDatabase() const {
        return _pdatabase;
    }
...
//        return 0;
    ...
    std::string GetDefaultViewerType() {
        ...
        tools::shared_ptr<RaveDatabase> pdatabase = _pdatabase;
        if( !!pdatabase ) {
...
private:
    static tools::shared_ptr<RaveGlobal> _state;
    // state that is always present

    // state that is initialized/destroyed
    tools::shared_ptr<RaveDatabase> _pdatabase;
    int _nDebugLevel;
...
tools::shared_ptr<RaveGlobal> RaveGlobal::_state;
...
{
    RaveGlobal::_state = tools::dynamic_pointer_cast<RaveGlobal>(globalstate);
}

UserDataPtr RaveGlobalState()
{
    // only return valid pointer if initialized!
    tools::shared_ptr<RaveGlobal> state = RaveGlobal::_state;
...
MultiControllerBasePtr RaveCreateMultiController(EnvironmentBasePtr env, const std::string& rawname)
{
    ...
    if( name == "genericmulticontroller" ) {
        return tools::static_pointer_cast<MultiControllerBase>(pcontroller);
    }
...
// Dummy Reader
DummyXMLReader::DummyXMLReader(const std::string& fieldname, const std::string& pparentname, tools::shared_ptr<std::ostream> osrecord) : _fieldname(fieldname), _osrecord(osrecord)
{
...
class CustomSamplerCallbackData : public tools::enable_shared_from_this<CustomSamplerCallbackData>, public UserData
{
public:
...
typedef tools::shared_ptr<CustomSamplerCallbackData> CustomSamplerCallbackDataPtr;
...
int SpaceSamplerBase::_CallStatusFunctions(int sampleiteration)
{
    ...
    FOREACHC(it,__listRegisteredCallbacks) {
        CustomSamplerCallbackDataPtr pitdata = tools::dynamic_pointer_cast<CustomSamplerCallbackData>(it->lock());
        if( !!pitdata) {
