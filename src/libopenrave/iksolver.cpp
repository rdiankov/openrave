...
class CustomIkSolverFilterData : public tools::enable_shared_from_this<CustomIkSolverFilterData>, public UserData
{
public:
...
typedef tools::shared_ptr<CustomIkSolverFilterData> CustomIkSolverFilterDataPtr;

class IkSolverFinishCallbackData : public tools::enable_shared_from_this<IkSolverFinishCallbackData>, public UserData
{
public:
...
typedef tools::shared_ptr<IkSolverFinishCallbackData> IkSolverFinishCallbackDataPtr;

bool CustomIkSolverFilterDataCompare(UserDataPtr data0, UserDataPtr data1)
{
    return tools::dynamic_pointer_cast<CustomIkSolverFilterData>(data0)->_priority > tools::dynamic_pointer_cast<CustomIkSolverFilterData>(data1)->_priority;
}

bool IkSolverBase::Solve(const IkParameterization& param, const std::vector<dReal>& q0, int filteroptions, IkReturnPtr ikreturn)
{
    if( !ikreturn ) {
        return Solve(param,q0,filteroptions,tools::shared_ptr< vector<dReal> >());
    }
    ikreturn->Clear();
    tools::shared_ptr< vector<dReal> > psolution(&ikreturn->_vsolution, utils::null_deleter());
...
{
    if( !ikreturn ) {
        return Solve(param,q0,vFreeParameters,filteroptions,tools::shared_ptr< vector<dReal> >());
    }
    ikreturn->Clear();
    tools::shared_ptr< vector<dReal> > psolution(&ikreturn->_vsolution, utils::null_deleter());
...
UserDataPtr IkSolverBase::RegisterCustomFilter(int32_t priority, const IkSolverBase::IkFilterCallbackFn &filterfn)
{
    ...
    FORIT(it, __listRegisteredFilters) {
        CustomIkSolverFilterDataPtr pitdata = tools::dynamic_pointer_cast<CustomIkSolverFilterData>(it->lock());
...
void IkSolverBase::_CallFinishCallbacks(IkReturnPtr ikreturn, RobotBase::ManipulatorConstPtr pmanip, const IkParameterization& ikparam)
{
    FOREACH(it, __listRegisteredFinishCallbacks) {
        IkSolverFinishCallbackDataPtr pitdata = tools::dynamic_pointer_cast<IkSolverFinishCallbackData>(it->lock());
        if( !!pitdata ) {
