...
void KinBody::LinkInfo::Reset()
{
    ...
    _mapExtraGeometriesSafety.clear();
    _id.clear();
    _name.clear();
    _t = Transform();
...
void KinBody::LinkInfo::ConvertUnitScale(dReal fUnitScale)
{
    ...
    FOREACH(itextra, _mapExtraGeometriesSafety) {
        FOREACH(itgeometry, itextra->second) {
            (*itgeometry)->ConvertUnitScale(fUnitScale);
        }
    }
...
void KinBody::LinkInfo::SerializeJSON(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator, dReal fUnitScale, int options) const
{
    ...
    if(_mapExtraGeometriesSafety.size() > 0 ) {
        ...
        FOREACHC(im, _mapExtraGeometriesSafety) {
            rapidjson::Value geometriesValue;
            geometriesValue.SetArray();
...
void KinBody::LinkInfo::DeserializeJSON(const rapidjson::Value &value, dReal fUnitScale, int options)
{
    ...
    // Parse the "extraGeometries" groups. Each parsed geometry is routed by its _bIsSafetyGeometry flag:
    // safety geometries go into _mapExtraGeometriesSafety, the rest into _mapExtraGeometries. A given group
    // name ends up in at most one of the two maps (mixing safety/non-safety under one group name is not
    // supported, since the group-lookup APIs assume disjoint names).
...
const std::vector<KinBody::GeometryInfoPtr>& KinBody::Link::GetGeometriesFromGroup(const std::string& groupname) const
{
    ...
    }
    {
        ...
        for(const std::pair< const std::string, std::vector<KinBody::GeometryInfoPtr> >& grouppair : _info._mapExtraGeometriesSafety) {
            ssGroupNames << grouppair.first << ", ";
        }
...
static bool _HasSafetyGeometry(const std::vector<KinBody::GeometryInfoPtr>& geometries)
{
...
static std::map<std::string, std::vector<KinBody::GeometryInfoPtr>>& _EnsureMapExtraGeometries(KinBody::LinkInfo& info, const std::vector<KinBody::GeometryInfoPtr>& geometries)
{
    return (_HasSafetyGeometry(geometries) ? info._mapExtraGeometriesSafety : info._mapExtraGeometries);
}
...
void KinBody::Link::_SetGroupGeometriesNoPostprocess(const std::string& groupname, const std::vector<KinBody::GeometryInfoPtr>& geometries)
{
    ...
    std::map<std::string, std::vector<KinBody::GeometryInfoPtr>>& mapExtraGeometries = _EnsureMapExtraGeometries(_info, geometries);
...
void KinBody::Link::AddGeometry(KinBody::GeometryInfoPtr pginfo, bool addToGroups)
{
    ...
    if( ginfo._name.size() > 0 ) {
        ...
        if( addToGroups ) {
            std::map<std::string, std::vector<KinBody::GeometryInfoPtr>>& mapExtraGeometries = _EnsureMapExtraGeometries(_info, {pginfo});
            FOREACH(itgeometrygroup, mapExtraGeometries) {
...
void KinBody::Link::AddGeometryToGroup(KinBody::GeometryInfoPtr pginfo, const std::string& groupname)
{
    ...
    std::map<std::string, std::vector<KinBody::GeometryInfoPtr>>& mapExtraGeometries = _EnsureMapExtraGeometries(_info, {pginfo});
    std::map< std::string, std::vector<KinBody::GeometryInfoPtr> >::iterator it = mapExtraGeometries.find(groupname);
    if( it == mapExtraGeometries.end() ) {
...
void KinBody::Link::RemoveGeometryByName(const std::string& geometryname, bool removeFromAllGroups)
{
    ...
    if( removeFromAllGroups ) {
        ...
        FOREACH(itgeometrygroup, _info._mapExtraGeometriesSafety) {
            std::vector<KinBody::GeometryInfoPtr>::iterator itgeometryinfo2 = itgeometrygroup->second.begin();
            while(itgeometryinfo2 != itgeometrygroup->second.end()) {
                if( (*itgeometryinfo2)->_name == geometryname ) {
                    itgeometryinfo2 = itgeometrygroup->second.erase(itgeometryinfo2);
