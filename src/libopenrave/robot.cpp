...
void RobotBase::AttachedSensor::UpdateInfo(SensorBase::SensorType type)
{
    if( !!psensor ) {
        ...
        _info._sensorgeometry = std::const_pointer_cast<SensorBase::SensorGeometry>(psensor->GetSensorGeometry(type));
...
void RobotBase::GetGrabbedInfo(std::vector<RobotBase::GrabbedInfoPtr>& vgrabbedinfo) const
{
    ...
    for(size_t i = 0; i < vgrabbedinfo.size(); ++i) {
        GrabbedConstPtr pgrabbed = std::dynamic_pointer_cast<Grabbed const>(_vGrabbedBodies[i]);
