...
public:
    ...
    {
        pcamera=psensor;
        pdata = tools::static_pointer_cast<SensorBase::CameraSensorData>(pcamera->CreateSensorData(SensorBase::ST_Camera));
        geom = *tools::static_pointer_cast<SensorBase::CameraGeomData>(pcamera->GetSensorGeometry(SensorBase::ST_Camera));
    ...
    tools::shared_ptr<SensorBase::CameraSensorData> pdata;
    IplImage* img;
};
...
public:
    virtual void demothread(int argc, char ** argv) {
        ...
        std::vector< tools::shared_ptr<OpenRAVECamera> > vcameras;
        for( std::vector<SensorBasePtr>::iterator itsensor = allsensors.begin(); itsensor != allsensors.end(); ++itsensor) {
            if( (*itsensor)->Supports(SensorBase::ST_Camera) ) {
                ...
                vcameras.push_back(tools::shared_ptr<OpenRAVECamera>(new OpenRAVECamera(*itsensor)));
            }
        }
        while(IsOk()) {
