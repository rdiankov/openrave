...
public:
    ...
    {
        pcamera=psensor;
        pdata = std::static_pointer_cast<SensorBase::CameraSensorData>(pcamera->CreateSensorData(SensorBase::ST_Camera));
        geom = *std::static_pointer_cast<SensorBase::CameraGeomData>(pcamera->GetSensorGeometry(SensorBase::ST_Camera));
    ...
    std::shared_ptr<SensorBase::CameraSensorData> pdata;
    IplImage* img;
};
...
public:
    virtual void demothread(int argc, char ** argv) {
        ...
        std::vector< std::shared_ptr<OpenRAVECamera> > vcameras;
        for( std::vector<SensorBasePtr>::iterator itsensor = allsensors.begin(); itsensor != allsensors.end(); ++itsensor) {
            if( (*itsensor)->Supports(SensorBase::ST_Camera) ) {
                ...
                vcameras.push_back(std::shared_ptr<OpenRAVECamera>(new OpenRAVECamera(*itsensor)));
            }
        }
        while(IsOk()) {
