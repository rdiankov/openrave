...
class HRP2GraspingScene(metaclass.AutoReloader):
    ...
    def reloadcontroller(self):
        ...
        try:
            self.robotreal.SetController(self.orenvreal.CreateController('ROSOpenRAVE'),'trajectoryservice /hrp2/controller_session')
        finally:
    ...
    def loadscene(self,scenefilename='scenes/r602cerealmanip.env.xml',robotname='HRP2JSK',graspingppfile='hrp2_frootloops_visibility.pp'):
        self.target = None
        ...
        self.homevalues = array([ -2.86345789e-03,   4.97418863e-04,  -4.52529013e-01,
                                   8.72043371e-01,  -4.17762041e-01,   8.16814136e-04,
                                   0.00000000e+00,   3.43606574e-03,  -4.97418863e-04,
                                   -4.52249706e-01,   8.72011900e-01,  -4.17901635e-01,
                                   7.53982284e-04,   0.00000000e+00,   8.47067393e-04,
                                   2.84897187e-03,   1.59534463e-03,   2.57708761e-03,
                                   7.69527555e-02,  -1.91759229e-01,   2.73909390e-01,
                                   -2.02681810e-01,  -2.30580971e-01,   9.90253594e-03,
                                   -2.71063328e-01,   1.55539140e-01,   2.52415299e-01,
                                   -3.49661469e-01,  -4.49272335e-01,   3.51060212e-01,
                                   7.22945556e-02,  -1.47477672e-01,  -2.63864666e-01, 2.64033407e-01])
        ...
        self.sensor = [s for s in self.robotreal.GetSensors() if s.GetName()=='wristcam'][0]
        ...
        self.rhand = self.orenvreal.ReadRobotXMLFile('robots/hrp2rhandjsk.robot.xml')
        self.orenvreal.AddRobot(self.rhand)
        T = eye(4)
        T[2,3] = 100
...
#         end
    ...
    def waitrobot(self):
        ...
        while not self.robotreal.GetController().IsDone():
            self.robotreal.WaitForController(1.0)
    ...
    def movegripper(self,grippervalues,robot=None):
        ...
        self.trajectories.append(trajdata)
        self.waitrobot()
    ...
    def testsim(self):
        ...
        trajdata = manipprob.SendCommand('closefingers offset 0.3 ' + cmdstr)
        if trajdata is None:
            raise ValueError('failed to find visual feedback grasp')
        self.starttrajectory(trajdata)
        ...
        self.starttrajectory(trajdata)
        trajdata = manipprob.SendCommand('movetohandposition maxiter 1000 maxtries 1 seedik 4 matrix ' + ' '.join(str(f) for f in transpose(Tnewee[0:3,0:4]).flat) + cmdstr)
        if trajdata is None:
            print 'failed to find trajectory'
        self.starttrajectory(trajdata)
