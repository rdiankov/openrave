...
public:
    ...
    virtual void SetBodyGeometryGroup(const KinBodyConstPtr &pbody, const std::string& groupname) OPENRAVE_DUMMY_IMPLEMENTATION;
    ...
    virtual bool CheckCollision(const KinBodyConstPtr &pbody1, CollisionReportPtr report = CollisionReportPtr())=0;
    ...
    virtual bool CheckCollision(const KinBodyConstPtr &pbody1, const KinBodyConstPtr &pbody2, CollisionReportPtr report = CollisionReportPtr())=0;

    /// \brief checks collision of a link and a scene. Attached bodies are ignored. CO_ActiveDOFs option is ignored.
    virtual bool CheckCollision(const KinBody::LinkConstPtr &plink, CollisionReportPtr report = CollisionReportPtr())=0;

    /// \brief checks collision of two links. Attached bodies are ignored. CO_ActiveDOFs option is ignored.
    virtual bool CheckCollision(const KinBody::LinkConstPtr &plink1, const KinBody::LinkConstPtr &plink2, CollisionReportPtr report = CollisionReportPtr())=0;

    /// \brief checks collision of a link and a body. Attached bodies for pbody are respected. CO_ActiveDOFs option is ignored.
    virtual bool CheckCollision(const KinBody::LinkConstPtr &plink, const KinBodyConstPtr &pbody, CollisionReportPtr report = CollisionReportPtr())=0;

    /// \brief checks collision of a link and a scene. Attached bodies are ignored. CO_ActiveDOFs option is ignored.
    virtual bool CheckCollision(const KinBody::LinkConstPtr &plink, const std::vector<KinBodyConstPtr>& vbodyexcluded, const std::vector<KinBody::LinkConstPtr>& vlinkexcluded, CollisionReportPtr report = CollisionReportPtr())=0;
    ...
    virtual bool CheckCollision(const KinBodyConstPtr &pbody, const std::vector<KinBodyConstPtr>& vbodyexcluded, const std::vector<KinBody::LinkConstPtr>& vlinkexcluded, CollisionReportPtr report = CollisionReportPtr())=0;
    ...
    virtual bool CheckCollision(const RAY& ray, const KinBody::LinkConstPtr &plink, CollisionReportPtr report = CollisionReportPtr()) = 0;
    ...
    /// \param[out] report [optional] collision report to be filled with data about the collision. If a body was hit, CollisionReport::plink1 contains the hit link pointer.
    virtual bool CheckCollision(const RAY& ray, const KinBodyConstPtr &pbody, CollisionReportPtr report = CollisionReportPtr()) = 0;
    ...
    /// \param pbody the link to collide with. If CO_ActiveDOFs is set, will only check affected links of the body.
    /// \param[out] report [optional] collision report to be filled with data about the collision. If a body was hit, CollisionReport::plink1 contains the hit link pointer.
    virtual bool CheckCollision(const TriMesh& trimesh, const KinBodyConstPtr &pbody, CollisionReportPtr report = CollisionReportPtr()) OPENRAVE_DUMMY_IMPLEMENTATION;
    ...
    virtual bool CheckStandaloneSelfCollision(const KinBodyConstPtr &pbody, CollisionReportPtr report = CollisionReportPtr()) = 0;

    /// \brief Checks self collision of the link with the rest of the links with its parent
    ///
    /// Only checks KinBody::GetNonAdjacentLinks(), Links that are joined together are ignored.
    /// \param[out] report [optional] collision report to be filled with data about the collision.
    virtual bool CheckStandaloneSelfCollision(const KinBody::LinkConstPtr &plink, CollisionReportPtr report = CollisionReportPtr()) = 0;

    /// \deprecated (13/04/09)
    virtual bool CheckSelfCollision(const KinBodyConstPtr &pbody, CollisionReportPtr report = CollisionReportPtr()) RAVE_DEPRECATED
    {
    ...
    /// \deprecated (13/04/09)
    virtual bool CheckSelfCollision(const KinBody::LinkConstPtr &plink, CollisionReportPtr report = CollisionReportPtr()) RAVE_DEPRECATED
    {
        //RAVELOG_WARN("CollisionCheckerBase::CheckSelfCollision has been deprecated, please use CollisionCheckerBase::CheckStandaloneSelfCollision\n");
