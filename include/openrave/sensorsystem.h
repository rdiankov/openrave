...
public:
        ...
        virtual void copy(tools::shared_ptr<XMLData const> pdata) {
            *this = *pdata;
        }
...
public:
        BodyData(SensorSystemBasePtr psensorsystem, KinBodyPtr pbody, tools::shared_ptr<XMLData> initdata) : KinBody::ManageData(psensorsystem), _initdata(initdata), bPresent(false), bEnabled(true), bLock(false)
        {
            SetBody(pbody);
        }
...
protected:
        ...
        tools::shared_ptr<XMLData> _initdata;
    ...
    {
public:
        SimpleXMLReader(tools::shared_ptr<XMLData>);
...
protected:
        tools::shared_ptr<XMLData> _pdata;
        std::stringstream ss;
    };
...
protected:
    typedef std::pair<tools::shared_ptr<BodyData>, Transform > SNAPSHOT;
    typedef std::map<int,tools::shared_ptr<BodyData> > BODIES;
    virtual tools::shared_ptr<BodyData> CreateBodyData(KinBodyPtr pbody, tools::shared_ptr<XMLData const> pdata);
    virtual void _UpdateBodies(std::list<SNAPSHOT>& listbodies);
    virtual void _UpdateBodiesThread();

    virtual void SetRecentTransform(tools::shared_ptr<BodyData> pdata, const Transform& t) {
        pdata->tnew = t;
    }
