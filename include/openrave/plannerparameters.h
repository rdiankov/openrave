...
protected:
    ...
    ProcessElement startElement(const std::string& name, const AttributesList& atts)
    {
        ...
        if( name == "workspacetrajectory" ) {
            _pcurreader.reset(new planningutils::TrajectoryReader(_penv,workspacetraj,atts));
            _bProcessing = false;
            return PE_Support;
        }
    ...
    bool endElement(const std::string& name)
    {
        ...
        if( !!_pcurreader ) {
            if( _pcurreader->endElement(name) ) {
                planningutils::TrajectoryReaderPtr ptrajreader = boost::dynamic_pointer_cast<planningutils::TrajectoryReader>(_pcurreader);
                if( !!ptrajreader ) {
