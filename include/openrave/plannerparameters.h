...
public:
    ConstraintTrajectoryTimingParameters() : TrajectoryTimingParameters(), maxlinkspeed(0), maxlinkaccel(0), maxmanipspeed(0), maxmanipaccel(0), vConstraintManipDir(0,0,1), vConstraintGlobalDir(0,0,1), fCosManipAngleThresh(-1), mingripperdistance(0), velocitydistancethresh(0), maxmergeiterations(1000), minswitchtime(0.2),nshortcutcycles(1), fSearchVelAccelMult(0.8), durationImprovemetnCutoffRatio(0.001), _bCProcessing(false) {
        ...
        _vXMLParameters.push_back("durationimprovemetncutoffratio");
    }
    ...
    int nshortcutcycles; ///< the minimum number of times the shortcut cycle is repeted.
    ...
    dReal durationImprovemetnCutoffRatio; ///< Whenever shortcut is accepted, if change is less than diff/iterations, then do not do anymore shortcutting.
...
protected:
    ...
    virtual bool serialize(std::ostream& O, int options=0) const
    {
        ...
        O << "<durationimprovemetncutoffratio>" << durationImprovemetnCutoffRatio << "</durationimprovemetncutoffratio>" << std::endl;
        if( !(options & 1) ) {
            O << _sExtraParameters << std::endl;
        }
    ...
    virtual bool endElement(const std::string& name)
    {
        if( _bCProcessing ) {
            ...
            else if( name == "durationimprovemetncutoffratio" ) {
                _ss >> durationImprovemetnCutoffRatio;
            }
