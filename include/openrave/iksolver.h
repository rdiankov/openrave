...
public:
     ...
     */
    virtual bool Solve(const IkParameterization& param, const std::vector<dReal>& q0, int filteroptions, tools::shared_ptr< std::vector<dReal> > solution = tools::shared_ptr< std::vector<dReal> >()) = 0;
    ...
    /** Return a joint configuration for the given end effector transform.
        ...
        \return true if solution is found
     */
    virtual bool Solve(const IkParameterization& param, const std::vector<dReal>& q0, const std::vector<dReal>& vFreeParameters, int filteroptions, tools::shared_ptr< std::vector<dReal> > solution=tools::shared_ptr< std::vector<dReal> >()) = 0;
...
protected:
    inline IkSolverBasePtr shared_iksolver() {
        return tools::static_pointer_cast<IkSolverBase>(shared_from_this());
    }
    inline IkSolverBaseConstPtr shared_iksolver_const() const {
        return tools::static_pointer_cast<IkSolverBase const>(shared_from_this());
    }
