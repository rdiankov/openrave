...
bool inline LoadJsonValueByKey(const rapidjson::Value& v, const char* key, T& t) {
    if (!v.IsObject()) {
        throw OPENRAVE_EXCEPTION_FORMAT0("Cannot load value of non-object for key \"" + std::string(key) + "\".", OpenRAVE::ORE_InvalidArguments);
    }
...
T GetJsonValueByKey(const rapidjson::Value& v, const char* key, const U& t) {
    if (!v.IsObject()) {
        throw OPENRAVE_EXCEPTION_FORMAT0("Cannot get value of non-object for key \"" + std::string(key) + "\".", OpenRAVE::ORE_InvalidArguments);
    }
    if (v.HasMember(key)) {
