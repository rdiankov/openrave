// -*- coding: utf-8 -*-
// Copyright (C) 2012-2018 MUJIN Inc.
//
...
inline void ParseJson(rapidjson::Document& d, std::istream& is) {
    ...
    if (d.HasParseError()) {
        throw OpenraveJSONException(boost::str(boost::format("Json stream is invalid (offset %u) %s")%((unsigned)d.GetErrorOffset())%GetParseError_En(d.GetParseError())), MJE_Failed);
    }
}
