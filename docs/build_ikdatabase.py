...
def build(stats,options):
    ...
    ikfast_url = 'http://openrave.programmingvision.com/ordocs/en/openravepy-html/openravepy.ikfast-module.html'
...
A collection for robots and statistics for the compiled inverse kinematics files using `IKFast <%s>`. This database is automatically generated by the `OpenRAVE testing infrastructure <%s>`_. All possible IKs for each robot's manipulator are generated. The following `inverse kinematics types <%s>`_ are supported: **%s**. The database provides URL links to the robot files, genreated ik files, and test results history.
